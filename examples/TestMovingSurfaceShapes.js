...
requirejs(
  ...
  function (WorldWind, LayerManager) {
    ...
    // Surface ellipse
    var ellipse = new WorldWind.SurfaceEllipse(
    ...
    // Surface rectangle
    var rectangle = new WorldWind.SurfaceRectangle(
    ...
    // Surface Sector
    var surfaceSector = new WorldWind.SurfaceSector(
      new WorldWind.Sector(33, 37, -95, -90),
      attributes
    );
    ...
    var bigPolyline = new WorldWind.SurfacePolyline(
      bigPolylineBoundary,
      attributes
    );
    ...
    var bigRectangle = new WorldWind.SurfaceRectangle(
    ...
    var polarEllipse = new WorldWind.SurfaceEllipse(
      new WorldWind.Location(90, -110),
      1e5,
      5e6,
      90,
      attributes
    ...
    window.setInterval(function () {
      ...
      bigRectangle.maximumNumEdgeIntervals =
        flipNumberOfEdgeIntervals(bigRectangle);
      ...
      polarEllipse.polarThrottle === 10
        ? (polarEllipse.polarThrottle = 0)
        : (polarEllipse.polarThrottle = 10);
    ...
    function flipNumberOfEdgeIntervals(shape) {
      if (shape.maximumNumEdgeIntervals === 128) {
        // 128 is the default number of edges
    ...
    function cycleThroughPathTypes(shape) {
      if (shape.pathType === WorldWindConstants.GREAT_CIRCLE) {
        return WorldWindConstants.RHUMB_LINE;
      } else if (shape.pathType === WorldWindConstants.RHUMB_LINE) {
        return WorldWindConstants.LINEAR;
      } else {
        return WorldWindConstants.GREAT_CIRCLE;
      }
    }
    ...
    function shiftBoundaries(shape) {
      ...
      if (shape.shiftCounter === 5) {
        shape.shiftDirection === "east"
          ? (shape.shiftDirection = "west")
          : (shape.shiftDirection = "east");
