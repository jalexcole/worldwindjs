...
define([
  ...
  NEO.prototype.onDatasetClick = function (event) {
    ...
    var datasetCaps = this.findLayerCapsByTitle(
      this.datasetDropdown.wmsCapsDoc.capability.layers[0],
      datasetTitle
    );

    $("#datasetDropdownTitle").html(
      datasetCaps.title + "<span class='caret'></span>"
    );
  ...
  NEO.prototype.setLayer = function (layerTitle) {
    ...
    var layerCaps = this.findLayerCapsByTitle(
      this.datasetDropdown.wmsCapsDoc.capability.layers[0],
      layerTitle
    );
  ...
  NEO.prototype.addLayer = function (layerCaps) {
    if (layerCaps.name) {
      ...
      if (
        ...
        var timeSequence =
          config.timeSequences[config.timeSequences.length - 1];
  ...
  NEO.prototype.addLegend = function (legendCaps) {
    ...
    var screenOffset = new WorldWind.Offset(
