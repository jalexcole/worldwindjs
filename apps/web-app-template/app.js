...
$(document).ready(function () {
  ...
  class Globe {
    ...
    addLayerFromWms(serviceAddress, layerName, options) {
      ...
      let parseCapabilities = function (xml) {
        ...
        var layerConfig =
          WorldWind.WmsLayer.formLayerConfiguration(layerForDisplay);
    ...
    findLayerByName(name) {
      let layers = this.wwd.layers.filter(
    ...
    zoomToLayer(layer) {
      ...
      if (
        layerSector.maxLatitude >= 90 &&
        layerSector.minLatitude <= -90 &&
        layerSector.maxLongitude >= 180 &&
        layerSector.minLongitude <= -180
      ) {
        console.log(
          "zoomToLayer: The selected layer covers the full globe. No camera centering needed."
        );
        return;
      }
      // Obtain layer center
      ...
      function findLayerCenter(layerSector) {
        var centerLatitude =
          (layerSector.maxLatitude + layerSector.minLatitude) / 2;
        var centerLongitude =
          (layerSector.maxLongitude + layerSector.minLongitude) / 2;
        var layerCenter = new WorldWind.Location(
          centerLatitude,
          centerLongitude
        );
        return layerCenter;
      ...
      function computeZoomRange(layerSector) {
        var verticalBoundary =
          layerSector.maxLatitude - layerSector.minLatitude;
        var horizontalBoundary =
          layerSector.maxLongitude - layerSector.minLongitude;
        // Calculate diagonal angle between boundaries (simple pythagoras formula, we don't need to
        // consider vectors or great circles).
        var diagonalAngle = Math.sqrt(
          Math.pow(verticalBoundary, 2) + Math.pow(horizontalBoundary, 2)
        );
        ...
        if (diagonalAngle >= 180) {
          ...
          var diagonalArcLength =
            (diagonalAngle / 360) * (2 * 3.1416 * 6371000);
  ...
  function LayersViewModel(globe) {
    ...
    self.overlayLayers = ko.observableArray(
      globe.getLayers("overlay").reverse()
    );
