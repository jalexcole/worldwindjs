...
var DataGrid = function (data) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (longitude < this.minLon) this.minLon = longitude;
    if (longitude > this.maxLon) this.maxLon = longitude;
    if (latitude < this.minLat) this.minLat = latitude;
    if (latitude > this.maxLat) this.maxLat = latitude;

    // Recognize when the first row of the grid ends and use that to calculate the grid width.
    if (firstLatitude === -1e5) {
      // identify the first latitude in the grid
    ...
    if (rawPosition[2] != "NaN") {
      ...
      this.positions.push(
        new WorldWind.Position(latitude, longitude, altitude * 1000)
      );
    }
...
DataGrid.prototype.lookupValue = function (latitude, longitude) {
  ...
  var colNW = Math.floor(
      ((this.width - 1) * (longitude - this.minLon)) /
        (this.maxLon - this.minLon)
    ),
    rowNW = Math.floor(
      ((this.height - 1) * (this.maxLat - latitude)) /
  ...
  if (swIndex && seIndex && nwIndex && neIndex) {
    var a = this.positions[swIndex],
      aa = a.altitude,
      b = this.positions[seIndex],
      bb = b.altitude,
      c = this.positions[nwIndex],
      cc = c.altitude,
      d = this.positions[neIndex],
      dd = d.altitude;
