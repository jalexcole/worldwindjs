...
define([
  ...
  var USGSSlabs = function () {
    ...
    this.loadSlabData(
      "CAS",
      "cascadia_slab1.0_clip.xyz",
      401,
      WorldWind.Color.YELLOW
    );
    this.loadSlabData(
      "SOL",
      "sol_slab1.0_clip.xyz",
      1001,
      WorldWind.Color.YELLOW
    );
    this.loadSlabData(
      "MEX",
      "mex_slab1.0_clip.xyz",
      1251,
  ...
  USGSSlabs.prototype.addGridToWorldWindow = function (name, dataGrid, color) {
    ...
    var splitShapes = WorldWind.TriangleMesh.split(
      dataGrid.positions,
      indices,
      null,
      null
    ...
    for (var i = 0; i < splitShapes.length; i++) {
      var mesh = new WorldWind.TriangleMesh(
        splitShapes[i].positions,
        splitShapes[i].indices,
        meshAttributes
      );
      mesh.altitudeMode = WorldWindConstants.ABSOLUTE;
          ...
          if (pickedObject.userObject instanceof WorldWind.TriangleMesh) {
            if (pickedObject.position) {
              var latitude = pickedObject.position.latitude,
                ...
                altitude = pickedObject.userObject.dataGrid.lookupValue(
                  latitude,
                  longitude
                );
              if (altitude !== null) {
                ...
                this.screenText.text =
                  Math.floor(Math.abs(altitude) / 1000).toString() + " Km";
