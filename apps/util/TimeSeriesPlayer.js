...
TimeSeriesPlayer.prototype.createPlayer = function () {
  var topButtonGroup = $('<div class="btn-group btn-group-vertical"></div>'),
    backwardButton = $(
      '<button id="timeSeriesBackward" type="button" class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-step-backward"></span></button>'
    ),
    playButton = $(
      '<button id="timeSeriesPlay" type="button" class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-play"></span></button>'
    ),
    forwardButton = $(
      '<button id="timeSeriesForward" type="button" class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-step-forward"></span></button>'
    ),
    bottomButtonGroup = $('<div class="btn-group btn-group-vertical"></div>'),
    repeatButton = $(
      '<button id="timeSeriesRepeat" type="button" class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-repeat"></span></button>'
    ),
    sliderDiv = $(
      '<div id="timeSeriesSlider" style="margin-top: 10px; margin-left: 10px; margin-bottom: 10px; background: #337ab7;"></div>'
    ),
    displaySpan = $(
      '<span id="timeSeriesDisplay" style="position: absolute; width: 300px; color: yellow; text-shadow: 0px 0px 2px black"></span>'
    );
...
TimeSeriesPlayer.prototype.onStepBackwardButton = function (event) {
  if (!this.isPlaying) {
    if (this.timeSequence) {
      var previousTime =
        this.timeSequence.previous() || this.timeSequence.previous();
...
TimeSeriesPlayer.prototype.onRepeatButton = function (event) {
  ...
  this.repeatButton
    .find("span")
    .removeClass(this.isRepeating ? "glyphicon-repeat" : "glyphicon-refresh");
  this.repeatButton
    .find("span")
    .addClass(this.isRepeating ? "glyphicon-refresh" : "glyphicon-repeat");
};

TimeSeriesPlayer.prototype.onSlide = function (event, ui) {
  if (!this.isPlaying && this.timeSequence) {
    this.timeSequence.currentTime = this.timeSequence.getTimeForScale(
      ui.value / 100
    );
...
TimeSeriesPlayer.prototype.updatePlayButton = function () {
  this.playButton
    .find("span")
    .removeClass(this.isPlaying ? "glyphicon-play" : "glyphicon-stop");
  this.playButton
    .find("span")
    .addClass(this.isPlaying ? "glyphicon-stop" : "glyphicon-play");
};
...
TimeSeriesPlayer.prototype.updateTimeDisplay = function (timeString) {
  ...
  this.timeDisplay.css(
    "top",
    this.sliderThumb.offset().top -
      this.slider.offset().top +
      this.slider.height()
  );
  this.timeDisplay.css(
    "left",
    this.sliderThumb.offset().left - this.slider.offset().left + 40
  );
};
