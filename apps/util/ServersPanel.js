  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4 && request.status === 200) {
      ...
      if (!xmlDom && request.responseText.indexOf("<?xml") === 0) {
        xmlDom = new window.DOMParser().parseFromString(
      ...
      if (!xmlDom) {
        alert(
          serverAddress + " retrieval failed. It is probably not a WMS server."
        );
        return;
      }
      ...
      if (wmsCapsDoc.version) {
        // if no version, then the URL doesn't point to a caps doc.
      ...
      if (request.statusText) {
        ...
        alert(
          "Failed to retrieve WMS capabilities from " + serverAddress + "."
        );
      }
    }
  };
...
ServersPanel.prototype.addServerPanel = function (serverAddress, wmsCapsDoc) {
  var treeId = this.idCounter++,
    ...
    panelTitle =
      wmsService.title && wmsService.title.length > 0
        ? wmsService.title
        : serverAddress;
  //
  ...
  var topDiv = $('<div class="panel panel-default"></div>'),
    heading = $(
      '<div class="panel-heading" role="tab" id="' + headingID + '"></div>'
    ),
    ...
    remove = $(
      '<a href="#"><small><span class="pull-right glyphicon glyphicon-remove clickable_space"></span></small></a>'
    ),
    bodyParent = $(
      '<div id="' +
        collapseID +
    ...
    body = $(
      '<div style="max-height: 250px; overflow-y: scroll; -webkit-overflow-scrolling: touch;"></div>'
    ),
  ...
  if (
    layers.length === 1 &&
    layers[0].layers &&
    layers[0].title === wmsCapsDoc.service.title &&
    !(layers[0].name && layers[0].name.length > 0)
  ) {
...
ServersPanel.prototype.addLayer = function (layerCaps) {
  if (layerCaps.name) {
    ...
    if (
      config.timeSequences &&
      config.timeSequences[config.timeSequences.length - 1] instanceof
        WorldWind.PeriodicTimeSequence
    ) {
      ...
      this.timeSeriesPlayer.timeSequence = new WorldWind.BasicTimeSequence(
        config.timeSequences
      );
      this.timeSeriesPlayer.layer = layer;
      layer.timeSequence = timeSequence;
    ...
    if (
      layerCaps.styles &&
      layerCaps.styles.length > 0 &&
      layerCaps.styles[0].legendUrls &&
      layerCaps.styles[0].legendUrls.length > 0
    ) {
