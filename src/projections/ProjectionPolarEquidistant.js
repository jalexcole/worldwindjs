 ...
 */
class ProjectionPolarEquidistant extends GeographicProjection {
  constructor(pole) {
    super( "Polar Equidistant", false, null);
  ...
  geographicToCartesian(globe,
    ...
    if ((this.north && latitude === 90) || (!this.north && latitude === -90)) {
      ...
      var northSouthFactor = this.north ? -1 : 1, a = globe.equatorialRadius *
        (Math.PI / 2 + latitude * Angle.DEGREES_TO_RADIANS * northSouthFactor);

      result[0] = a * Math.sin(longitude * Angle.DEGREES_TO_RADIANS);
      result[1] =
        a * Math.cos(longitude * Angle.DEGREES_TO_RADIANS) * northSouthFactor;
      result[2] = elevation;
    }
  ...
  geographicToCartesianGrid(globe,
    ...
    var eqr = globe.equatorialRadius, minLat = sector.minLatitude * Angle.DEGREES_TO_RADIANS, maxLat = sector.maxLatitude * Angle.DEGREES_TO_RADIANS, minLon = sector.minLongitude * Angle.DEGREES_TO_RADIANS, maxLon = sector.maxLongitude * Angle.DEGREES_TO_RADIANS, deltaLat = (maxLat - minLat) / (numLat > 1 ? numLat - 1 : 1), deltaLon = (maxLon - minLon) / (numLon > 1 ? numLon - 1 : 1), northSouthFactor = this.north ? -1 : 1, refPoint = referencePoint ? referencePoint : new Vec3(0, 0, 0), pi_2 = Math.PI / 2, latIndex, lonIndex, elevIndex = 0, resultIndex = 0, cosLon = new Float64Array(numLon), sinLon = new Float64Array(numLon), lat, lon, a;
    ...
    for (latIndex = 0, lat = minLat; latIndex < numLat; latIndex++, lat += deltaLat) {
      ...
      for (lonIndex = 0; lonIndex < numLon; lonIndex++) {
        ...
        result[resultIndex++] =
          a * cosLon[lonIndex] * northSouthFactor - refPoint[1];
  ...
  cartesianToGeographic(globe,
    ...
    if (rho < 1.0e-4) {
      ...
      result.latitude =
        Math.asin(Math.cos(c) * (this.north ? 1 : -1)) * Angle.RADIANS_TO_DEGREES;
      result.longitude =
        Math.atan2(x, y * (this.north ? -1 : 1)) * Angle.RADIANS_TO_DEGREES; // use atan2(x,y) instead of atan(x/y)
      result.altitude = z;
    }
  ...
  northTangentAtLocation(globe,
    ...
    result[0] =
      Math.sin(longitude * Angle.DEGREES_TO_RADIANS) * (this.north ? -1 : 1);
...
// ProjectionPolarEquidistant.prototype = Object.create(
//   GeographicProjection.prototype
// );
