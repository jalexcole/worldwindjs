 ...
 */
class ProjectionGnomonic extends GeographicProjection {
  /**
   * Constructs a polar gnomonic geographic projection.
   * @param {String} pole Indicates the north or south aspect. Specify "North" for the north aspect or "South"
   * for the south aspect.
   * Initializes the projection with the appropriate display name and geographic limits based on the pole aspect.
   * Internally stores the pole for further use and sets a unique state key.
   */
    ...
    if ((this.north && latitude === 90) || (!this.north && latitude === -90)) {
      ...
      var poleFactor = this.north ? 1 : -1,
        a =
          ...
          Math.tan(latitude * Angle.DEGREES_TO_RADIANS); // R cot(phi)

      result[0] =
        a * Math.sin(longitude * Angle.DEGREES_TO_RADIANS) * poleFactor; // eqs. 22-6, 22-10
  ...
  geographicToCartesianGrid(
    ...
    var minLat = sector.minLatitude * Angle.DEGREES_TO_RADIANS,
      ...
      minLatLimit =
        this.projectionLimits.minLatitude * Angle.DEGREES_TO_RADIANS,
      maxLatLimit =
        this.projectionLimits.maxLatitude * Angle.DEGREES_TO_RADIANS,
    ...
    for (
      ...
      if (
        (this.north && clampedLat === Math.PI / 2) ||
        (!this.north && clampedLat === -Math.PI / 2)
      ) {
        a = 0;
      }
  ...
  cartesianToGeographic(globe, x, y, z, offset, result) {
    ...
    if (rho < 1.0e-4) {
      ...
      result.latitude =
        Math.asin(Math.cos(c) * (this.north ? 1 : -1)) *
        Angle.RADIANS_TO_DEGREES; // eq. 20-14
      result.longitude =
        Math.atan2(x, y * (this.north ? -1 : 1)) * Angle.RADIANS_TO_DEGREES; // use atan2(x,y) instead of atan(x/y). 20-16, 20-17
      result.altitude = z;
    }
  ...
  set pole(pole) {
    ...
    this.projectionLimits = this.north
      ? new Sector(30, 90, -180, 180)
      : new Sector(-90, -30, -180, 180);
    this._stateKey = "projection polar gnomonic " + this._pole + " ";
  }
