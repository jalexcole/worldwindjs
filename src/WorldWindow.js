...
class WorldWindow {
  ...
  cameraFromLookAt(lookAt) {
    ...
    var elevation =
      globe.elevationAtLocation(position.latitude, position.longitude) * ve +
      10.0; // 10m above surface
   ...
   */
  cameraAsLookAt(result, terrainPosition) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
  ...
  pickShapesInRegion(rectangle) {
    ...
    this.drawContext.pickRectangle = new Rectangle(
      rectangle.x,
      this.canvas.height - rectangle.y,
      rectangle.width,
      rectangle.height
    );
    this.drawFrame();
  ...
  handleContextLost(event) {
    Logger.log(
      Logger.LEVEL_INFO,
      "WebGL context event: " + event.statusMessage
    );
  ...
  computeViewingTransform(projection, modelview) {
    ...
    if (projection) {
      ...
      var nearDistance = WWMath.perspectiveNearDistanceForFarDistance(
        farDistance,
        10,
        this.depthBits
      );
  ...
  cameraToViewingTransform(modelview) {
    ...
    modelview.multiplyByFirstPersonModelview(
   ...
   * @ignore
   */
  lookAtToViewingTransform(lookAt, modelview) {
    if (!lookAt) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
          "WorldWindow",
          "lookAtToViewingTransform",
          "missingGlobe"
        )
      );
    }
    ...
    modelview.setToIdentity();
    modelview.multiplyByLookAtModelview(
      lookAt.position,
      lookAt.range,
      lookAt.heading,
      lookAt.tilt,
  ...
  pixelSizeAtDistance(distance) {
    var tanfovy_2 = Math.tan(
      ((this.camera.fieldOfView * 0.5) / 180.0) * Math.PI
    );
  ...
  computeDrawContext() {
    ...
    dc.modelviewNormalTransform = Matrix.fromIdentity().setToTransposeOfMatrix(
      modelviewInv.upper3By3()
    );
  ...
  resetDrawContext() {
    ...
    dc.horizonDistance = this.globe.horizonDistance(
      this.camera.position.altitude
    );
  ...
  doDraw() {
    ...
    if (this.subsurfaceMode && this.hasStencilBuffer) {
      ...
      this.drawContext.currentGlContext.disable(
      ...
      if (!this.deferOrderedRendering) {
        ...
        this.drawContext.currentGlContext.stencilFunc(
          this.drawContext.currentGlContext.ALWAYS,
          1,
          1
        );
        this.drawContext.currentGlContext.stencilOp(
          this.drawContext.currentGlContext.REPLACE,
          this.drawContext.currentGlContext.REPLACE,
          this.drawContext.currentGlContext.REPLACE
        );
  ...
  redrawSurface() {
    ...
    this.drawContext.currentGlContext.stencilFunc(
      this.drawContext.currentGlContext.EQUAL,
      1,
      1
    );
    this.drawContext.currentGlContext.stencilOp(
      this.drawContext.currentGlContext.KEEP,
      this.drawContext.currentGlContext.KEEP,
      this.drawContext.currentGlContext.KEEP
    );
    this.drawContext.surfaceShapeTileBuilder.clear();
  ...
  createTerrain() {
    ...
    dc.frameStatistics.setTerrainTileCount(
      dc.terrain ? dc.terrain.surfaceGeometry.length : 0
    );
  }
  makeCurrent(offset) {
    var dc = this.drawContext;
  ...
  resolveRegionPick() {
    ...
    var pickedObjects = this.drawContext.objectsAtPickPoint,
      uniquePickColors = this.drawContext.readPickColors(
  ...
  callRedrawCallbacks(stage) {
    for (var i = 0, len = this._redrawCallbacks.length; i < len; i++) {
      try {
        ...
        Logger.log(
          Logger.LEVEL_SEVERE,
          "Exception calling redraw callback.\n" + e.toString()
        );
   ...
   */
  cartesianToScreenPoint(x, y, z, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.ERROR,
    ...
    if (
      this.scratchProjection.project(x, y, z, this.viewport, this.scratchPoint)
    ) {
   ...
   * @throws {ArgumentError} If the result is null
   */
  geographicToScreenPoint(latitude, longitude, altitude, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.ERROR,
    ...
    return this.cartesianToScreenPoint(
      this.scratchPoint[0],
      this.scratchPoint[1],
      this.scratchPoint[2],
      result
  ...
  rayThroughScreenPoint(point) {
    ...
    modelviewProjection.setToMultiply(
      this.scratchProjection,
      this.scratchModelview
    );
    ...
    if (
      !modelviewProjectionInv.unProject(screenPoint, this.viewport, nearPoint)
    ) {
      return null;
    }
    ...
    screenPoint[2] = 1;
    if (
      !modelviewProjectionInv.unProject(screenPoint, this.viewport, farPoint)
    ) {
      return null;
    }
