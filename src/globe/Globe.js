...
class Globe {
  constructor(elevationModel, projection) {
    if (!elevationModel) {
      throw new ArgumentError(
        Logger.logMessage(
          ...
          "Globe",
          "constructor",
          "Elevation model is null or undefined."
        )
      );
    }
    /**
     ...
     */
    this.equatorialRadius = WorldWindConstants.WGS84_SEMI_MAJOR_AXIS;
    var f = 1 / WorldWindConstants.WGS84_INVERSE_FLATTENING;
  ...
  static idPool = 0; // Used to assign unique IDs to globes for use in their state keys.
   ...
   */
  computePointFromPosition(latitude, longitude, altitude, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    return this.projection.geographicToCartesian(
      this,
      latitude,
      longitude,
      altitude,
   ...
   * @throws {ArgumentError} If the specified result argument is null or undefined.
   */
  computePointFromLocation(latitude, longitude, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
  ...
  computePointsForGrid(
    ...
    return this.projection.geographicToCartesianGrid(
      this,
      sector,
      numLat,
      numLon,
   ...
   * @throws {ArgumentError} If the specified result argument is null or undefined.
   */
  computePositionFromPoint(x, y, z, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    this.projection.cartesianToGeographic(
      this,
      x,
      y,
      z,
   ...
   */
  radiusAt(latitude, longitude) {
    var sinLat = Math.sin(latitude * Angle.DEGREES_TO_RADIANS),
      rpm =
        this.equatorialRadius /
        Math.sqrt(1.0 - this.eccentricitySquared * sinLat * sinLat);
   ...
   * @throws {ArgumentError} If the specified result argument is null or undefined.
   */
  surfaceNormalAtLocation(latitude, longitude, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    if (this.projection.surfaceNormalAtLocation) {
      return this.projection.surfaceNormalAtLocation(
        this,
        latitude,
        longitude,
        result
   ...
   * @throws {ArgumentError} If the specified result argument is null or undefined.
   */
  surfaceNormalAtPoint(x, y, z, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
   ...
   * @throws {ArgumentError} If the specified result argument is null or undefined.
   */
  northTangentAtLocation(latitude, longitude, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    return this.projection.northTangentAtLocation(
      this,
      latitude,
      longitude,
      result
   ...
   * @throws {ArgumentError} If the specified result argument is null or undefined.
   */
  northTangentAtPoint(x, y, z, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    return this.projection.northTangentAtPoint(
      this,
      x,
      y,
      z,
  ...
  intersectsFrustum(frustum) {
    ...
    if (this.is2D()) {
      ...
      bbox.setToSector(
        Sector.FULL_SPHERE,
        this,
        this.elevationModel.minElevation,
   ...
   */
  elevationsForGrid(sector, numLat, numLon, targetResolution, result) {
    if (!sector) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    if (numLat <= 0 || numLon <= 0) {
      throw new ArgumentError(
        Logger.logMessage(
          ...
          "Globe",
          "elevationsForSector",
          "numLat or numLon is less than 1"
        )
      );
    }
    ...
    return this.elevationModel.elevationsForGrid(
      sector,
      numLat,
      numLon,
      targetResolution,
