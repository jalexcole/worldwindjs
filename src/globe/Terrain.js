 ...
 */
class Terrain {
  constructor(globe, tessellator, terrainTiles, verticalExaggeration) {
    /**
   ...
   */
  surfacePoint(latitude, longitude, offset, result) {
    if (!result) {
    ...
    for (var i = 0, len = this.surfaceGeometry.length; i < len; i++) {
      if (
        this.surfaceGeometry[i].sector.containsLocation(latitude, longitude)
      ) {
        this.surfaceGeometry[i].surfacePoint(latitude, longitude, result);

        if (offset) {
          var normal = this.globe.surfaceNormalAtPoint(
            result[0],
            result[1],
            result[2],
            Terrain.scratchPoint
          );
  ...
  surfacePointForMode(latitude, longitude, offset, altitudeMode, result) {
    ...
    if (!altitudeMode) altitudeMode = WorldWindConstants.ABSOLUTE;

    if (altitudeMode === WorldWindConstants.CLAMP_TO_GROUND) {
      return this.surfacePoint(latitude, longitude, 0, result);
    } else if (altitudeMode === WorldWindConstants.RELATIVE_TO_GROUND) {
