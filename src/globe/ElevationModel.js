...
class ElevationModel {
     ...
     * @ignore
     */
    coverageComparator(coverage1, coverage2) {
     ...
     * @ignore
     */
    performCoverageListChangedActions() {
    ...
    elevationAtLocation(latitude, longitude) {
        ...
        for (i = n - 1; i >= 0; i--) {
            var coverage = this.coverages[i];
            if (coverage.enabled &&
                coverage.coverageSector.containsLocation(latitude, longitude)) {
     ...
     * @ignore
     */
    preferredCoverageIndex(sector,
        location,
        targetResolution) {
        var i, n = this.coverages.length, minResDiff = Number.MAX_VALUE, minDiffIdx = -1;

        for (i = 0; i < n; i++) {
            var coverage = this.coverages[i], validCoverage = coverage.enabled &&
                ((sector !== null && coverage.coverageSector.intersects(sector)) ||
                    (location !== null &&
                        coverage.coverageSector.containsLocation(
                            location.latitude,
                            location.longitude
                        )));
            if (validCoverage) {
     ...
     */
    bestCoverageAtLocation(latitude,
        longitude,
        targetResolution) {
        ...
        var preferredIndex = this.preferredCoverageIndex(
            null,
            this.scratchLocation,
            targetResolution
        );
        if (preferredIndex >= 0) {
    ...
    elevationsForGrid(sector,
        ...
        result.fill(NaN);
        var resolution = Number.MAX_VALUE, resultFilled = false, preferredIndex = this.preferredCoverageIndex(
            sector,
            null,
            targetResolution
        );
        ...
        if (preferredIndex >= 0) {
            for (var i = preferredIndex; !resultFilled && i >= 0; i--) {
                var coverage = this.coverages[i];
                if (coverage.enabled && coverage.coverageSector.intersects(sector)) {
                    resultFilled = coverage.elevationsForGrid(
                        sector,
                        numLat,
                        numLon,
                        result
