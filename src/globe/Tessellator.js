...
import TerrainTileList from "./TerrainTileList";
...
class Tessellator {
  ...
  tessellate(dc) {
    ...
    if (!this.topLevelTiles[dc.globeStateKey] ||
      this.topLevelTiles[dc.globeStateKey].length == 0) {
  ...
  createTile(tileSector, level, row, column) {
    if (!tileSector) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
  ...
  beginRendering(dc) {
    ...
    var gl = dc.currentGlContext, gpuResourceCache = dc.gpuResourceCache;
  ...
  beginRenderingTile(dc, terrainTile) {
    ...
    this.scratchMatrix.setToMultiply(
      dc.modelviewProjection,
      terrainTile.transformationMatrix
    );
    dc.currentProgram.loadModelviewProjection(gl, this.scratchMatrix);

    var vboCacheKey = dc.globeStateKey + terrainTile.tileKey, vbo = gpuResourceCache.resourceForKey(vboCacheKey);
    if (!vbo) {
      vbo = gl.createBuffer();
  ...
  renderTile(dc, terrainTile) {
    ...
    var gl = dc.currentGlContext, prim = gl.TRIANGLE_STRIP; // replace TRIANGLE_STRIP with LINE_STRIP to debug borders
    ...
    var level = terrainTile.level, neighborLevel;
  ...
  pick(dc, tileList, pickDelegate) {
    ...
    var color = null, userObject = pickDelegate || this, position = new Position(0, 0, 0), pickableTiles = [];
    ...
    if (!dc.regionPicking) {
      ...
      if (point) {
        ...
        position.altitude = dc.globe.elevationAtLocation(
          position.latitude,
          position.longitude
        );
        dc.addPickedObject(
          new PickedObject(color, userObject, position, null, true)
        );
      }
    }
  }
  // Internal function. Intentionally not documented.
  drawPickTiles(dc, tileList, color) {
    var gl = dc.currentGlContext;
  ...
  // Internal function. Intentionally not documented.
  computeNearestIntersection(line, tileList) {
    ...
    if (results.length == 0) {
      ...
      var minDistance = Number.POSITIVE_INFINITY, minIndex;
  ...
  // Internal function. Intentionally not documented.
  computeIntersections(line, tile, results) {
    var level = tile.level, neighborLevel, points = tile.points, elements, firstResult = results.length;
  ...
  addTile(dc, tile) {
    ...
    // Corners of the tile.
    var neTileCorner = [sector.maxLatitude, sector.maxLongitude].toString(), seTileCorner = [sector.minLatitude, sector.maxLongitude].toString(), nwTileCorner = [sector.maxLatitude, sector.minLongitude].toString(), swTileCorner = [sector.minLatitude, sector.minLongitude].toString(), corner;
  ...
  refineNeighbors(dc) {
    ...
    for (var idx = 0, len = this.tiles.length; idx < len; idx += 1) {
      var tile = this.tiles[idx], levelNumber = tile.level.levelNumber, sector = tile.sector, corner, neighbor, idx, len;

      // Corners of the tile.
      var neTileCorner = [sector.maxLatitude, sector.maxLongitude].toString(), seTileCorner = [sector.minLatitude, sector.maxLongitude].toString(), nwTileCorner = [sector.maxLatitude, sector.minLongitude].toString(), swTileCorner = [sector.minLatitude, sector.minLongitude].toString();
    ...
    var tilesNeedingRefinement = [], tilesNotNeedingRefinement = [];
  ...
  setNeighbors(tile) {
    ...
    var neCorner = this.corners[neTileCorner], seCorner = this.corners[seTileCorner], nwCorner = this.corners[nwTileCorner], swCorner = this.corners[swTileCorner];
  ...
  isTileVisible(dc, tile) {
    if (dc.globe.projectionLimits &&
      !tile.sector.overlaps(dc.globe.projectionLimits)) {
      return false;
    }

    return tile.extent.intersectsFrustum(dc.frustumInModelCoordinates);
  }
  tileMeetsRenderCriteria(dc, tile) {
  ...
  regenerateTileGeometryIfNeeded(dc, tile) {
   ...
   * @ignore
   */
  coverageTargetResolution(texelSize) {
    return (texelSize / 8) * Angle.RADIANS_TO_DEGREES;
  }
  regenerateTileGeometry(dc, tile) {
    var numLat = tile.tileHeight + 1, // num points in each dimension is 1 more than the number of tile cells
      numLon = tile.tileWidth + 1, refPoint = tile.referencePoint, elevations = this.scratchElevations;
    ...
    dc.globe.elevationsForGrid(
      tile.sector,
      numLat,
      numLon,
      this.coverageTargetResolution(tile.texelSize),
      elevations
    );
    ...
    tile.transformationMatrix.setTranslation(
      refPoint[0],
      refPoint[1],
      refPoint[2]
    );
  }
  mustAlignNeighborElevations(dc, tile) {
    var level = tile.level, northLevel = tile.neighborLevel(WorldWindConstants.NORTH), southLevel = tile.neighborLevel(WorldWindConstants.SOUTH), eastLevel = tile.neighborLevel(WorldWindConstants.EAST), westLevel = tile.neighborLevel(WorldWindConstants.WEST);
  ...
  alignNeighborElevations(dc,
    ...
    var numLat = tile.tileHeight + 1, // num points in each dimension is 1 more than the number of tile cells
      ...
      prevNumLon = Math.floor(numLon / 2) + 1, prevLevel = level.previousLevel(), prevElevations = this.scratchPrevElevations, neighborLevel, i, index, prevIndex;
    ...
    dc.globe.elevationsForGrid(
      ...
      prevNumLat,
      prevNumLon,
      this.coverageTargetResolution(prevLevel.texelSize),
      prevElevations
    );
    ...
    if (neighborLevel && neighborLevel.compare(level) < 0) {
      ...
      for (i = 0; i < prevNumLon; i++, index += 2, prevIndex += 1) {
        ...
        if (i < prevNumLon - 1) {
          elevations[index + 1] =
            0.5 * (prevElevations[prevIndex] + prevElevations[prevIndex + 1]);
        }
      }
    }
    ...
    if (neighborLevel && neighborLevel.compare(level) < 0) {
      ...
      for (i = 0; i < prevNumLat; i++, index += 2 * numLon, prevIndex += prevNumLon) {
        elevations[index] = prevElevations[prevIndex];
        if (i < prevNumLat - 1) {
          elevations[index + numLon] =
            0.5 *
            (prevElevations[prevIndex] + prevElevations[prevIndex + prevNumLon]);
        }
      }
    }
  ...
  buildSharedGeometry() {
    ...
    var tileWidth = this.levels.tileWidth, tileHeight = this.levels.tileHeight;
  ...
  buildTexCoords(tileWidth, tileHeight) {
    var numCols = tileWidth + 1, numRows = tileHeight + 1, colDelta = 1 / tileWidth, rowDelta = 1 / tileHeight, buffer = new Float32Array(numCols * numRows * 2), index = 0;
  ...
  buildIndices(tileWidth, tileHeight) {
    ...
    this.indicesLoresNorth = new Uint16Array(
      indices.slice(this.indicesLoresNorthOffset)
    );
    ...
    this.indicesLoresSouth = new Uint16Array(
      indices.slice(this.indicesLoresSouthOffset)
    );
    ...
    this.indicesLoresWest = new Uint16Array(
      indices.slice(this.indicesLoresWestOffset)
    );
    ...
    this.indicesLoresEast = new Uint16Array(
      indices.slice(this.indicesLoresEastOffset)
    );
  ...
  buildWireframeIndices(tileWidth, tileHeight) {
    ...
    var index = 0, lonIndex, latIndex, vertexIndex;
  ...
  buildOutlineIndices(tileWidth, tileHeight) {
  ...
  cacheSharedGeometryVBOs(dc) {
    ...
    if (!texCoordVbo) {
      ...
      dc.frameStatistics.incrementVboLoadCount(1);
      gpuResourceCache.putResource(
        this.texCoordVboCacheKey,
        texCoordVbo,
        (this.texCoords.length * 4) / 2
      );
    }
