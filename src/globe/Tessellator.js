...
Tessellator.prototype.tessellate = function (dc) {
  ...
  for (
    var index = 0, len = this.topLevelTiles[dc.globeStateKey].length;
    index < len;
    index += 1
  ) {
...
Tessellator.prototype.addTileDescendants = function (dc, tile) {
  ...
  for (var index = 0; index < subTiles.length; index += 1) {
    ...
    if (
      this.levels.sector.intersects(child.sector) &&
...
Tessellator.prototype.alignNeighborElevations = function (
  dc,
  tile,
  elevations
) {
  ...
  if (neighborLevel && neighborLevel.compare(level) < 0) {
    ...
    for (
      i = 0;
      i < prevNumLat;
      i++, index += 2 * numLon, prevIndex += prevNumLon
    ) {
      elevations[index] = prevElevations[prevIndex];
      if (i < prevNumLat - 1) {
...
Tessellator.prototype.buildIndices = function (tileWidth, tileHeight) {
  ...
  var numIndices =
    2 * (numLatVertices - 3) * (numLonVertices - 2) + 2 * (numLatVertices - 3);
  var indices = [];
...
Tessellator.prototype.buildWireframeIndices = function (tileWidth, tileHeight) {
  ...
  var numIndices =
    2 * tileWidth * numLatVertices + 2 * tileHeight * numLonVertices;
  var indices = [];
