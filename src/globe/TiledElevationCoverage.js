...
import ElevationCoverage from "./ElevationCoverage";
 ...
 */
class TiledElevationCoverage extends ElevationCoverage{
  constructor(config) {
    super(config.resolution);
    if (!config) {
    ...
    var firstLevelDelta = 45, tileWidth = 256, lastLevel = LevelSet.numLevelsForResolution(
      firstLevelDelta / tileWidth,
      config.resolution
    ...
    this.levels = new LevelSet(
      this.coverageSector,
      new Location(firstLevelDelta, firstLevelDelta),
      numLevels,
      tileWidth,
      tileWidth
    );
  ...
  minAndMaxElevationsForSector(sector,
    ...
    var level = this.levels.levelForTexelSize(
      (sector.deltaLatitude() * Angle.DEGREES_TO_RADIANS) / 64
    );
  ...
  pointElevationForLocation(latitude,
    longitude) {
    var level = this.levels.lastLevel(), deltaLat = level.tileDelta.latitude, deltaLon = level.tileDelta.longitude, r = Tile.computeRow(deltaLat, latitude), c = Tile.computeColumn(deltaLon, longitude), tileKey, image = null;
  ...
  areaElevationsForGrid(sector,
    numLat,
    numLon,
    level,
    result) {
    var minLat = sector.minLatitude, maxLat = sector.maxLatitude, minLon = sector.minLongitude, maxLon = sector.maxLongitude, deltaLat = sector.deltaLatitude() / (numLat > 1 ? numLat - 1 : 1), deltaLon = sector.deltaLongitude() / (numLon > 1 ? numLon - 1 : 1), lat, lon, s, t, latIndex, lonIndex, resultIndex = 0;
  ...
  areaElevationForCoord(s,
    t,
    levelNumber,
    result,
    resultIndex) {
    var level, levelWidth, levelHeight, tMin, tMax, vMin, vMax, u, v, x0, x1, y0, y1, xf, yf, retrieveTiles, pixels = new Float64Array(4);

    for (var i = levelNumber; i >= 0; i--) {
      level = this.levels.level(i);
      levelWidth = Math.round(
        (level.tileWidth * 360) / level.tileDelta.longitude
      );
      levelHeight = Math.round(
        (level.tileHeight * 180) / level.tileDelta.latitude
      );
  ...
  lookupPixels(x0,
    ...
    retrieveTiles,
    result) {
    var levelNumber = level.levelNumber, tileWidth = level.tileWidth, tileHeight = level.tileHeight, row0 = Math.floor(y0 / tileHeight), row1 = Math.floor(y1 / tileHeight), col0 = Math.floor(x0 / tileWidth), col1 = Math.floor(x1 / tileWidth), r0c0, r0c1, r1c0, r1c1;
  ...
  lookupImage(levelNumber,
    row,
    column,
    retrieveTiles) {
    var tileKey = Tile.computeTileKey(levelNumber, row, column), image = this.imageCache.entryForKey(tileKey);
  ...
  addAncestor(level,
    ...
    column,
    retrieveTiles) {
    var tile = null, r = Math.floor(row / 2), c = Math.floor(column / 2);
  ...
  // Intentionally not documented.
  tileForLevel(levelNumber,
    row,
    column) {
    var tileKey = Tile.computeTileKey(levelNumber, row, column), tile = this.tileCache.entryForKey(tileKey);

    if (tile) {
      return tile;
    }

    var level = this.levels.level(levelNumber), sector = Tile.computeSector(level, row, column);
  ...
  // Intentionally not documented.
  isTileImageInMemory(tile) {
    return this.imageCache.containsKey(tile.tileKey);
  }
  // Intentionally not documented.
  resourceUrlForTile(tile) {
  ...
  retrieveTileImage(tile) {
    if (this.currentRetrievals.length < this.retrievalQueueSize &&
      ...
      var url = this.resourceUrlForTile(tile, this.retrievalImageFormat), xhr = new XMLHttpRequest(), elevationCoverage = this;
        ...
        if (xhr.readyState === 4) {
          ...
          if (xhr.status === 200) {
            if (contentType === elevationCoverage.retrievalImageFormat ||
              ...
              Logger.log(
                Logger.LEVEL_INFO,
                "Elevations retrieval succeeded: " + url
              );
              elevationCoverage.loadElevationImage(tile, xhr);
              elevationCoverage.absentResourceList.unmarkResourceAbsent(
                tile.tileKey
              );
      ...
      xhr.ontimeout = function () {
        ...
        Logger.log(
          Logger.LEVEL_WARNING,
          "Elevations retrieval timed out: " + url
        );
      };
  ...
  // Intentionally not documented
  removeFromCurrentRetrievals(tileKey) {
  ...
  loadElevationImage(tile, xhr) {
    var elevationImage = new ElevationImage(
      tile.sector,
      tile.tileWidth,
      tile.tileHeight
    ), geoTiff;
    ...
    if (this.retrievalImageFormat === "application/bil16") {
      ...
      elevationImage.size =
        (elevationImage.imageData.length * geoTiff.metadata.bitsPerSample[0]) / 8;
    }
