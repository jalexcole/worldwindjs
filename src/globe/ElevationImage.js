...
class ElevationImage {
    constructor(sector, imageWidth, imageHeight) {
        if (!sector) {
     ...
     * @ignore
     */
    static isNoData(x0y0, x1y0, x0y1, x1y1) {
    ...
    elevationsForGrid(sector,
        ...
        for (j = 0, lat = minLat; j < numLat; j += 1, lat += deltaLat) {
            ...
            if (lat >= minLatSelf && lat <= maxLatSelf) {
                ...
                var y = ((this.imageHeight - 1) * (maxLatSelf - lat)) / deltaLatSelf, y0 = Math.floor(WWMath.clamp(y, 0, this.imageHeight - 1)), y1 = Math.floor(WWMath.clamp(y0 + 1, 0, this.imageHeight - 1)), yf = y - y0;
                ...
                for (i = 0, lon = minLon; i < numLon; i += 1, lon += deltaLon) {
                    ...
                    if (lon >= minLonSelf && lon <= maxLonSelf && isNaN(result[index])) {
                        ...
                        var x = ((this.imageWidth - 1) * (lon - minLonSelf)) / deltaLonSelf, x0 = Math.floor(WWMath.clamp(x, 0, this.imageWidth - 1)), x1 = Math.floor(WWMath.clamp(x0 + 1, 0, this.imageWidth - 1)), xf = x - x0;

                        var x0y0 = pixels[x0 + y0 * this.imageWidth], x1y0 = pixels[x1 + y0 * this.imageWidth], x0y1 = pixels[x0 + y1 * this.imageWidth], x1y1 = pixels[x1 + y1 * this.imageWidth];
    ...
    minAndMaxElevationsForSector(sector) {
        ...
        if (!sector) {
            ...
            var maxLatSelf = this.sector.maxLatitude, minLonSelf = this.sector.minLongitude, deltaLatSelf = this.sector.deltaLatitude(), deltaLonSelf = this.sector.deltaLongitude(), minLatOther = sector.minLatitude, maxLatOther = sector.maxLatitude, minLonOther = sector.minLongitude, maxLonOther = sector.maxLongitude;
            ...
            var minY = Math.floor(
                ((this.imageHeight - 1) * (maxLatSelf - maxLatOther)) / deltaLatSelf
            ), maxY = Math.ceil(
                ((this.imageHeight - 1) * (maxLatSelf - minLatOther)) / deltaLatSelf
            ), minX = Math.floor(
                ((this.imageWidth - 1) * (minLonOther - minLonSelf)) / deltaLonSelf
            ), maxX = Math.ceil(
                ((this.imageWidth - 1) * (maxLonOther - minLonSelf)) / deltaLonSelf
            );
            ...
            var pixels = this.imageData, min = Number.MAX_VALUE, max = -min;
    ...
    findMinAndMaxElevation() {
        ...
        if (this.imageData && this.imageData.length > 0) {
            ...
            var pixels = this.imageData, pixelCount = this.imageWidth * this.imageHeight;
