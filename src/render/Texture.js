...
class Texture {
  constructor(gl, image, wrapMode) {
    ...
    this.texParameters = {};
    this.texParameters[gl.TEXTURE_MIN_FILTER] = isPowerOfTwo
  ...
  applyTexParameters(dc) {
    ...
    var textureMinFilter = dc.pickingMode
      ? gl.NEAREST
      : this.texParameters[gl.TEXTURE_MIN_FILTER] || gl.LINEAR;
    ...
    var textureMagFilter = dc.pickingMode
      ? gl.NEAREST
      : this.texParameters[gl.TEXTURE_MAG_FILTER] || gl.LINEAR;
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, textureMagFilter);

    gl.texParameteri(
      gl.TEXTURE_2D,
      gl.TEXTURE_WRAP_S,
      this.texParameters[gl.TEXTURE_WRAP_S] || gl.CLAMP_TO_EDGE
    );
    gl.texParameteri(
      gl.TEXTURE_2D,
      gl.TEXTURE_WRAP_T,
      this.texParameters[gl.TEXTURE_WRAP_T] || gl.CLAMP_TO_EDGE
    );
    ...
    if (textureMagFilter === gl.LINEAR) {
      ...
      if (this.anisotropicFilterExt) {
        gl.texParameteri(
          gl.TEXTURE_2D,
          this.anisotropicFilterExt.TEXTURE_MAX_ANISOTROPY_EXT,
          4
        );
      }
    }
