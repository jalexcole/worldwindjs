...
class DrawContext {
  ...
  reset() {
    ...
    this.timestamp = Date.now();
    if (this.timestamp === previousTimestamp) ++this.timestamp;
  ...
  update() {
    ...
    this.screenProjection.setToScreenProjection(
  ...
  bindFramebuffer(framebuffer) {
    if (this.currentFramebuffer != framebuffer) {
      this.currentGlContext.bindFramebuffer(
        this.currentGlContext.FRAMEBUFFER,
   ...
   */
  findAndBindProgram(programConstructor) {
    ...
    if (program) {
      ...
      try {
        ...
        this.bindProgram(program);
        this.gpuResourceCache.putResource(
          programConstructor.key,
          program,
          program.size
        );
      } catch (e) {
        Logger.log(
          Logger.LEVEL_SEVERE,
          "Error attempting to create GPU program."
        );
      }
    }
   ...
   */
  readPickColor(pickPoint) {
    var glPickPoint = this.convertPointToViewport(pickPoint, new Vec2(0, 0)),
      colorBytes = new Uint8Array(4);

    this.currentGlContext.readPixels(
      glPickPoint[0],
      glPickPoint[1],
      1,
      1,
      this.currentGlContext.RGBA,
      this.currentGlContext.UNSIGNED_BYTE,
      colorBytes
    );
   ...
   */
  readPickColors(pickRectangle) {
    var gl = this.currentGlContext,
      colorBytes = new Uint8Array(
        pickRectangle.width * pickRectangle.height * 4
      ),
      uniqueColors = {},
      color,
    ...
    for (
      var i = 0, len = pickRectangle.width * pickRectangle.height;
      i < len;
      i++
    ) {
      var k = i * 4;
      color = Color.colorFromBytes(
        colorBytes[k],
        colorBytes[k + 1],
        colorBytes[k + 2],
        colorBytes[k + 3]
   ...
   * @returns {null}
   */
  resolvePick(pickableObject) {
    if (
      !(pickableObject.userObject instanceof SurfaceShape) &&
      this.deepPicking &&
      !this.regionPicking
    ) {
  ...
  makePickFrustum() {
    ...
    var lln,
      ...
      ulf,
      urn,
      urf, // corner points of frustum
      nl,
      nr,
      nt,
      nb,
      nn,
      nf, // normal vectors of frustum planes
      l,
      r,
      t,
      b,
      ...
      f, // frustum planes
      va,
      vb = new Vec3(0, 0, 0), // vectors formed by the corner points
    ...
    if (
      xr < 0 ||
      yt < 0 ||
      xl > viewport.x + viewport.width ||
      yb > viewport.y + viewport.height
    ) {
      return false; // pick rectangle is outside the viewport.
    }

    pickRectangle.x = WWMath.clamp(xl, viewport.x, viewport.x + viewport.width);
    pickRectangle.y = WWMath.clamp(
      yb,
      viewport.y,
      viewport.y + viewport.height
    );
    pickRectangle.width =
      WWMath.clamp(xr, viewport.x, viewport.x + viewport.width) -
      pickRectangle.x;
    pickRectangle.height =
      WWMath.clamp(yt, viewport.y, viewport.y + viewport.height) -
      pickRectangle.y;
    ...
    screenPoint[2] = 0;
    modelviewProjectionInv.unProject(
      screenPoint,
      viewport,
      (lln = new Vec3(0, 0, 0))
    );
   ...
   */
  unitCubeElements() {
    var vboId = this.gpuResourceCache.resourceForKey(
      DrawContext.unitCubeElementsKey
    );
   ...
   * WorldWindConstants.ABSOLUTE, WorldWindConstants.CLAMP_TO_GROUND and
   * WorldWindConstants.RELATIVE_TO_GROUND. The mode WorldWindConstants.ABSOLUTE is used if the
   ...
   */
  surfacePointForMode(latitude, longitude, offset, altitudeMode, result) {
    if (!result) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    if (this.terrain) {
      this.terrain.surfacePointForMode(
        latitude,
        longitude,
        offset,
        altitudeMode,
   ...
   * @throws {ArgumentError} If either the specified point or result argument is null or undefined.
   */
  projectWithDepth(modelPoint, depthOffset, result) {
    if (!modelPoint) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
   ...
   */
  convertPointToViewport(point, result) {
    if (!point) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
   ...
   */
  createTextTexture(text, textAttributes) {
   ...
   */
  computeTextTextureStateKey(text, attributes) {
    if (!text || !attributes) {
      return null;
    }
    ...
    return (
      text +
      "c " +
      attributes.color.toHexString(true) +
      " f " +
      attributes.font.toString() +
      " eo " +
      attributes.enableOutline +
      " ow " +
      attributes.outlineWidth +
      " oc " +
      attributes.outlineColor.toHexString(true)
    );
  }
  /**
  ...
  getExtension(extensionName) {
    ...
    if (!(extensionName in this.glExtensionsCache)) {
      this.glExtensionsCache[extensionName] =
        this.currentGlContext.getExtension(extensionName) || null;
    }
