...
import FramebufferTile from "./FramebufferTile";
...
class FramebufferTileController {
  ...
  render(dc) {
    ...
    var gl = dc.currentGlContext, framebuffer = dc.currentFramebuffer;
    try {
  ...
  // Internal. Intentionally not documented.
  assembleTiles(dc) {
    var timestamp = dc.timestamp, globeStateKey = dc.globeStateKey;
  ...
  addTileOrDescendants(dc, tile) {
    ...
    var subTiles = tile.subdivideToCache(
