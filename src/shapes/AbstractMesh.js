 ...
 */
class AbstractMesh extends AbstractShape {
  ...
  doMakeOrderedRenderable(dc) {
    ...
    dc.surfacePointForMode(
      ...
      this.referencePosition.altitude * this._altitudeScale,
      this._altitudeMode,
      refPt
    );
   ...
   * @protected
   */
  computeMeshIndices(currentData) {
    throw new UnsupportedOperationError(
      Logger.logMessage(
        Logger.LEVEL_SEVERE,
        "AbstractMesh",
   ...
   * @protected
   */
  computeOutlineIndices(currentData) {
    // Default implementation does nothing.
  }
  // Internal. Intentionally not documented.
  computeNormals(currentData) {
    ...
    for (var i = 0; i < indices.length; i += 3) {
      ...
      var n = Vec3.computeTriangleNormal(
        triPoints[0],
        triPoints[1],
        triPoints[2]
      );
  ...
  doRenderOrdered(dc) {
    ...
    if (this.activeAttributes.drawInterior) {
      var applyLighting =
        !dc.pickingMode &&
        currentData.normals &&
        this.activeAttributes.applyLighting;
      ...
      if (hasTexture && (!dc.pickingMode || !this.pickTransparentImagePixels)) {
        ...
        if (textureBound) {
          if (!currentData.texCoordsVboCacheKey) {
            currentData.texCoordsVboCacheKey =
              dc.gpuResourceCache.generateCacheKey();
          }
      ...
      if (applyLighting) {
        ...
        if (!currentData.normalsVboCacheKey) {
          currentData.normalsVboCacheKey =
            dc.gpuResourceCache.generateCacheKey();
        }
    ...
    if (this.activeAttributes.drawOutline && this.meshOutlineIndices) {
      ...
      if (!currentData.outlineIndicesVboCacheKey) {
        currentData.outlineIndicesVboCacheKey =
          dc.gpuResourceCache.generateCacheKey();
      }
      ...
      gl.drawElements(
        gl.LINE_STRIP,
        this.meshOutlineIndices.length,
        gl.UNSIGNED_SHORT,
        0
      );
    }
  ...
  computePickPosition(dc) {
    var currentData = this.currentData,
      line = dc.pickRay,
      localLineOrigin = new Vec3(
        line.origin[0],
        line.origin[1],
        line.origin[2]
      ).subtract(currentData.referencePoint),
