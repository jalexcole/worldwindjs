...
Placemark.prototype.renderOrdered = function (dc) {
  ...
  if (dc.pickingMode) {
    ...
    if (dc.pickPoint && this.mustDrawLabel()) {
      if (
        this.labelBounds.containsPoint(
...
Placemark.prototype.makeOrderedRenderable = function (dc) {
  ...
  if (
    !dc.projectWithDepth(this.placePoint, depthOffset, Placemark.screenPoint)
  ) {
    return null;
  }
 ...
 */
Placemark.prototype.selectLevelOfDetail = function (
  dc,
  placemark,
  cameraDistance
) {
...
Placemark.prototype.doDrawOrderedPlacemark = function (dc) {
  ...
  if (!dc.pickingMode && this.mustDrawLabel()) {
    if (this.currentVisibility != this.targetVisibility) {
      var visibilityDelta =
        (dc.timestamp - dc.previousRedrawTimestamp) / dc.fadeTime;
      if (this.currentVisibility < this.targetVisibility) {
