...
import PlacemarkAttributes from "./PlacemarkAttributes";
 ...
 */
class Placemark extends Renderable {
  constructor(position, eyeDistanceScaling, attributes) {
    super();
    if (!position) {
     ...
     * @type {number}
     */
    this.eyeDistanceScalingLabelThreshold =
      1.5 * this.eyeDistanceScalingThreshold;
  ...
  makeOrderedRenderable(dc) {
    ...
    this.eyeDistance = this.alwaysOnTop
      ? 0
      : dc.eyePoint.distanceTo(this.placePoint);
    ...
    var visibilityScale = this.eyeDistanceScaling
      ? Math.max(
        0.0,
        Math.min(1, this.eyeDistanceScalingThreshold / this.eyeDistance)
      )
      : 1;
    ...
    if (this.imageTilt) {
      var actualTilt = this.imageTiltReference === WorldWindConstants.RELATIVE_TO_GLOBE
    ...
    if (this.imageRotation) {
      var actualRotation = this.imageRotationReference === WorldWindConstants.RELATIVE_TO_GLOBE
    ...
    if (this.mustDrawLabel()) {
      this.labelTexture = dc.createTextTexture(
        this.label,
        this.activeAttributes.labelAttributes
      );
    ...
    if (this.activeAttributes && this.activeAttributes.imageSource) {
      ...
      if (!this.activeTexture || this.updateImage) {
        this.activeTexture = dc.gpuResourceCache.retrieveTexture(
          ...
          this.activeAttributes.imageSource,
          null,
          this.activeAttributes.imageInitialWidth,
          this.activeAttributes.imageInitialHeight
        );
  ...
  isVisible(dc) {
    if (dc.pickingMode) {
      return (
        dc.pickRectangle &&
        (this.imageBounds.intersects(dc.pickRectangle) ||
          (this.mustDrawLabel() &&
            this.labelBounds.intersects(dc.pickRectangle)) ||
          (this.mustDrawLeaderLine(dc) &&
            dc.pickFrustum.intersectsSegment(this.groundPoint, this.placePoint)))
      );
    } else {
      return (
        this.imageBounds.intersects(dc.viewport) ||
        (this.mustDrawLabel() && this.labelBounds.intersects(dc.viewport)) ||
        (this.mustDrawLeaderLine(dc) &&
          dc.frustumInModelCoordinates.intersectsSegment(
            this.groundPoint,
            this.placePoint
          ))
      );
  ...
  // Internal. Intentionally not documented.
  doDrawOrderedPlacemark(dc) {
    var gl = dc.currentGlContext, program = dc.currentProgram, depthTest = true, textureBound;

    if (dc.pickingMode) {
      this.pickColor = dc.uniquePickColor();
    }

    if (this.eyeDistanceScaling &&
      this.eyeDistance > this.eyeDistanceScalingLabelThreshold) {
    ...
    if (!dc.pickingMode && this.mustDrawLabel()) {
      if (this.currentVisibility != this.targetVisibility) {
        ...
        if (this.currentVisibility < this.targetVisibility) {
          this.currentVisibility = Math.min(
            1,
            this.currentVisibility + visibilityDelta
          );
        } else {
          this.currentVisibility = Math.max(
            0,
            this.currentVisibility - visibilityDelta
          );
        }
    ...
    if (this.mustDrawLeaderLine(dc)) {
      ...
      var leaderLineVboId = dc.gpuResourceCache.resourceForKey(
        this.leaderLineCacheKey
      );
      if (!leaderLineVboId) {
    ...
    if (this.mustDrawLabel() && this.currentVisibility > 0) {
      program.loadOpacity(
        gl,
        dc.pickingMode ? 1 : this.layer.opacity * this.currentVisibility
      );
  ...
  mustDrawLabel() {
    return (
      this.label && this.label.length > 0 && this.activeAttributes.labelAttributes
    );
  }
  // Internal. Intentionally not documented.
  mustDrawLeaderLine(dc) {
    return (
      this.activeAttributes.drawLeaderLine &&
      this.activeAttributes.leaderLineAttributes &&
