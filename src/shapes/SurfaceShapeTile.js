 ...
 *
 */
class SurfaceShapeTile extends TextureTile {
  ...
  addSurfaceShape(surfaceShape) {
    ...
    this.surfaceShapeStateKeys.push(
      surfaceShape.stateKey + " lo " + surfaceShape.layer.opacity
    ); // combine the shape state key with layer opacity
  }
  // Internal use only. Intentionally not documented.
  needsUpdate(dc) {
    ...
    for (idx = 0, len = this.surfaceShapes.length; idx < len; idx += 1) {
      if (this.surfaceShapeStateKeys[idx] !==
        this.asRenderedSurfaceShapeStateKeys[idx]) {
        return true;
      }
    }
   ...
   */
  updateTexture(dc) {
    var gl = dc.currentGlContext, canvas = SurfaceShapeTile.canvas, ctx2D = SurfaceShapeTile.ctx2D;
    ...
    var xScale = this.tileWidth / this.sector.deltaLongitude(), yScale = -this.tileHeight / this.sector.deltaLatitude(), xOffset = -this.sector.minLongitude * xScale, yOffset = -this.sector.maxLatitude * yScale;
  ...
  getCacheKey() {
    if (!this.cacheKey) {
      this.cacheKey =
        "SurfaceShapeTile:" + this.tileKey + "," + this.pickSequence.toString();
    }
