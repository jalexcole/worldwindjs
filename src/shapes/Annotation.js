 ...
 */
class Annotation extends Renderable {
  static matrix = Matrix.fromIdentity();
  static screenPoint = new Vec3(0, 0, 0);
  ...
  makeOrderedRenderable(dc) {
    var w, h, s, iLeft, iRight, iTop, iBottom, offset, leaderGapHeight;
    ...
    offset = this.calloutOffset.offsetForSize(
      (w + iLeft + iRight) * s,
      (h + iTop + iBottom) * s
    );
    ...
    if (this.attributes.stateKey !== this.lastStateKey) {
      this.calloutPoints = this.createCallout(
        ...
        leaderOffsetX,
        leaderOffsetY,
        this.attributes.leaderGapWidth,
  ...
  // Internal. Intentionally not documented.
  drawCorner(x0, y0, cornerRadius, start, end, steps, buffer, startIdx) {
    if (cornerRadius < 1) {
      return startIdx;
    }
  ...
  doDrawOrderedAnnotation(dc) {
    ...
    program.loadOpacity(
      gl,
      dc.pickingMode ? 1 : this.attributes.opacity * this.layer.opacity
    );
    ...
    var calloutAttributesChanged =
      this.attributes.stateKey !== this.lastStateKey;
    ...
    program.loadColor(
      gl,
      dc.pickingMode ? this.pickColor : this.attributes.backgroundColor
    );
