...
class SurfaceShapeTileBuilder {
   ...
   */
  insertSurfaceShape(surfaceShape) {
  ...
  doDeepPickingRender(dc) {
    ...
    var deepPickShapes = [];
    for (
      idxPick = 0, lenPicks = dc.objectsAtPickPoint.objects.length;
      idxPick < lenPicks;
      idxPick += 1
    ) {
    ...
    for (
      idxShape = 0, lenShapes = deepPickShapes.length;
      idxShape < lenShapes;
      idxShape += 1
    ) {
      shape = deepPickShapes[idxShape];
      for (
        idxTile = 0, lenTiles = this.surfaceShapeTiles.length;
        idxTile < lenTiles;
        idxTile += 1
      ) {
      ...
      if (!!pickColor && shape.pickColor.equals(pickColor)) {
        po = new PickedObject(
          shape.pickColor.clone(),
          shape.pickDelegate ? shape.pickDelegate : shape,
          null,
          shape.layer,
          false
        );
    ...
    // to the pick list.
    for (
      idxPick = 0, lenPicks = resolvablePickObjects.length;
      idxPick < lenPicks;
      idxPick += 1
    ) {
  ...
  buildTiles(dc) {
    ...
    this.surfaceShapes.splice(0, this.surfaceShapes.length);
    for (
      var idx = 0, len = this.surfaceShapeTiles.length;
      idx < len;
      idx += 1
    ) {
  ...
  assembleTiles(dc) {
    ...
    for (
      idxShape = 0, lenShapes = this.surfaceShapes.length;
      idxShape < lenShapes;
      idxShape += 1
    ) {
      ...
      for (
        idxSector = 0, lenSectors = sectors.length;
        idxSector < lenSectors;
        idxSector += 1
      ) {
        var sector = sectors[idxSector];

        for (
          idxTile = 0, lenTiles = this.topLevelTiles.length;
          idxTile < lenTiles;
          idxTile += 1
        ) {
   ...
   */
  addTileOrDescendants(dc, levels, parentTile, tile) {
    ...
    for (
      var idxTile = 0, lenTiles = subTiles.length;
      idxTile < lenTiles;
      idxTile += 1
    ) {
   ...
   */
  addIntersectingShapes(dc, parentTile, tile) {
    var shapes = parentTile.getShapes();
    for (
      var idxShape = 0, lenShapes = shapes.length;
      idxShape < lenShapes;
      idxShape += 1
    ) {
      ...
      for (
        var idxSector = 0, lenSectors = sectors.length;
        idxSector < lenSectors;
        idxSector += 1
      ) {
        var sector = sectors[idxSector];
  ...
  intersectsFrustum(dc, tile) {
    ...
    return tile.extent.intersectsFrustum(
      dc.pickingMode ? dc.pickFrustum : dc.frustumInModelCoordinates
    );
  }
  /**
   ...
   */
  meetsRenderCriteria(dc, levels, tile) {
    return (
      tile.level.levelNumber == levels.lastLevel().levelNumber ||
      !tile.mustSubdivide(dc, this.detailControl)
    );
  }
}
