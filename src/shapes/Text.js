...
class Text extends Renderable {
  ...
  drawMarker(dc) {
    ...
    var markerTexture = dc.gpuResourceCache.resourceForKey(
      this.markerImageSource
    );
    if (!markerTexture) {
      dc.gpuResourceCache.retrieveTexture(
        dc.currentGlContext,
        this.markerImageSource
      );
      return;
    ...
    if (!dc.pickingMode) {
      ...
      program.loadOpacity(
        gl,
        this.layer.opacity * (1 - this.currentVisibility)
      );
    }
    ...
    Text.matrix.multiplyByScale(
      markerTexture.imageWidth * s,
      markerTexture.imageHeight * s,
      1
    );
    program.loadModelviewProjection(gl, Text.matrix);
