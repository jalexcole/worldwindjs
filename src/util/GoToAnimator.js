...
class GoToAnimator {
  ...
  goTo(position, completionCallback) {
    ...
    var animationDuration = this.travelTime, panDistance = Location.greatCircleDistance(
      this.startPosition,
      this.targetPosition
    ), rangeDistance;
    ...
    var viewportSize = (this.wwd.pixelSizeAtDistance(this.startPosition.altitude) *
      this.wwd.canvas.clientWidth) /
      this.wwd.globe.equatorialRadius;
    ...
    this.maxAltitudeReachedTime =
      this.maxAltitude <= this.lookAt.range ? Date.now() : null;
    ...
    if (this.maxAltitude > this.startPosition.altitude) {
      ...
      rangeDistance = Math.abs(
        this.targetPosition.altitude - this.startPosition.altitude
      );
    }

    // Determine which distance governs the animation duration.
    var animationDistance = Math.max(
      panDistance,
      rangeDistance / this.wwd.globe.equatorialRadius
    );
    ...
    if (animationDistance < 2 * viewportSize) {
      ...
      animationDuration = Math.min(
        (animationDistance / viewportSize) * this.travelTime,
        this.travelTime
      );
    }
  ...
  updateRange(currentPosition) {
    ...
    var continueAnimation = false, nextRange, elapsedTime;
    ...
    if (!this.maxAltitudeReachedTime) {
      ...
      nextRange = Math.min(
        this.startPosition.altitude + this.rangeVelocity * elapsedTime,
        this.maxAltitude
      );
      ...
      // We're done if we get withing 1 meter of the desired range.
      continueAnimation =
        Math.abs(this.lookAt.range - this.targetPosition.altitude) > 1;
    }
  ...
  updateLocation(currentPosition) {
    ...
    var elapsedTime = Date.now() - this.startTime, distanceTravelled = Location.greatCircleDistance(
      this.startPosition,
      currentPosition
    ), distanceRemaining = Location.greatCircleDistance(
      currentPosition,
      this.targetPosition
    ), azimuthToTarget = Location.greatCircleAzimuth(
      currentPosition,
      this.targetPosition
    ), distanceForNow = this.panVelocity * elapsedTime, nextDistance = Math.min(
      distanceForNow - distanceTravelled,
      distanceRemaining
    ), nextLocation = Location.greatCircleLocation(
      currentPosition,
      azimuthToTarget,
      nextDistance,
      new Location(0, 0)
