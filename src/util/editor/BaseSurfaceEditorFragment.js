 ...
 */
BaseSurfaceEditorFragment.prototype.initializeControlElements = function (
  shape,
  controlPoints,
  shadowControlPoints,
  accessories,
 ...
 */
BaseSurfaceEditorFragment.prototype.updateControlElements = function (
  shape,
  globe,
  controlPoints,
  shadowControlPoints,
 ...
 * performed or not.
 */
BaseSurfaceEditorFragment.prototype.reshape = function (
  shape,
  globe,
  controlPoint,
  currentPosition,
 ...
 */
BaseSurfaceEditorFragment.prototype.addNewVertex = function (
  shape,
  globe,
  position
) {
...
BaseSurfaceEditorFragment.prototype.createControlPoint = function (
  controlPoints,
  attributes,
  purpose,
  index
...
BaseSurfaceEditorFragment.prototype.computeControlPointDelta = function (
  globe,
  positionA,
  positionB
) {
...
BaseSurfaceEditorFragment.prototype.createRotationAccessory = function (
  accessories,
  attributes
) {
...
BaseSurfaceEditorFragment.prototype.updateRotationAccessory = function (
  centerPosition,
...
BaseSurfaceEditorFragment.prototype.normalizedHeading = function (
  currentHeading,
  deltaHeading
) {
  var newHeading =
    currentHeading * Angle.DEGREES_TO_RADIANS +
    deltaHeading * Angle.DEGREES_TO_RADIANS;
  ...
  return (
    ...
    (newHeading >= 0 ? newHeading : newHeading + Angle.TWO_PI)
  );
};
  ...
  if (locations.length > 0 && Array.isArray(locations[0])) {
    for (var i = 0, ilen = locations.length; i < ilen; i++) {
      ...
      for (var j = 0, jlen = locations[i].length; j < jlen; j++) {
        ring.push(
          new Location(locations[i][j].latitude, locations[i][j].longitude)
        );
      }
    ...
    for (var i = 0, len = locations.length; i < len; i++) {
      newLocations.push(
        new Location(locations[i].latitude, locations[i].longitude)
      );
    }
  }
...
BaseSurfaceEditorFragment.prototype.getCenterFromLocations = function (
  globe,
  locations
) {
  var count = 0;
...
BaseSurfaceEditorFragment.prototype.getAverageDistance = function (
  globe,
  center,
  locations
) {
...
BaseSurfaceEditorFragment.prototype.moveLocation = function (
  globe,
  controlPoint,
  currentPosition,
  previousPosition,
...
BaseSurfaceEditorFragment.prototype.rotateLocations = function (
  globe,
  newPosition,
  previousPosition,
  locations
  ...
  var deltaHeading =
    Location.greatCircleAzimuth(center, newPosition) - previousHeading;
...
BaseSurfaceEditorFragment.prototype.closestPointOnSegment = function (
  segmentStart,
  segmentEnd,
  point
) {
...
BaseSurfaceEditorFragment.prototype.computeShadowPointLocations = function (
  shape,
  shadowControlPoint,
  startLocation,
  endLocation
  ...
  if (shape.pathType === WorldWindConstants.LINEAR) {
    ...
    shadowControlPoint.position = Location.rhumbLocation(
      startLocation,
      segmentAzimuth,
      0.5 * segmentDistance,
    ...
    if (segmentAzimuth == null) {
      ...
      segmentDistance = Location.greatCircleDistance(
        startLocation,
        endLocation
      ); //radians
    }
    //Location, degrees, radians, Location
    shadowControlPoint.position = Location.greatCircleLocation(
      startLocation,
      segmentAzimuth,
      0.5 * segmentDistance,
      shadowControlPoint.position
