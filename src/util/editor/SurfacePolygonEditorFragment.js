...
SurfacePolygonEditorFragment.prototype = Object.create(
...
SurfacePolygonEditorFragment.prototype.createShadowShape = function (shape) {
  return new SurfacePolygon(
    this.deepCopyLocations(shape.boundaries),
    shape.attributes
  );
};

// Internal use only.
SurfacePolygonEditorFragment.prototype.getShapeCenter = function (
  shape,
  globe
) {
...
// Internal use only.
SurfacePolygonEditorFragment.prototype.initializeControlElements = function (
  shape,
  controlPoints,
  shadowControlPoints,
  accessories,
...
// Internal use only.
SurfacePolygonEditorFragment.prototype.updateControlElements = function (
  shape,
  globe,
  controlPoints,
  shadowControlPoints,
  ...
  for (var i = lenControlPoints - 1; i > -1; i--) {
    if (
      controlPoints[i].userProperties.purpose === ShapeEditorConstants.ROTATION
    ) {
      rotationControlPoint = controlPoints[i];

      var polygonCenter = this.getCenterFromLocations(globe, locations);
      var polygonRadius =
        1.2 * this.getAverageDistance(globe, polygonCenter, locations);
  ...
  if (locationControlPoints.length > lenLocations) {
    locationControlPoints.splice(
      lenLocations,
      locationControlPoints.length - lenLocations
    );
  }
  ...
  for (var i = 0; i < controlPoints.length - 1; i++) {
    ...
    this.computeShadowPointLocations(
      shape,
      shadowControlPoints[i],
      locations[i],
      locations[i + 1]
    );
...
// Internal use only.
SurfacePolygonEditorFragment.prototype.reshape = function (
  shape,
  globe,
  controlPoint,
  currentPosition,
  ...
  if (controlPoint.userProperties.purpose === ShapeEditorConstants.ROTATION) {
    var deltaHeading = this.rotateLocations(
      globe,
      currentPosition,
      previousPosition,
      locations
    );
    this.currentHeading = this.normalizedHeading(
      this.currentHeading,
      deltaHeading
    );
...
// Internal use only.
SurfacePolygonEditorFragment.prototype.addNewVertex = function (
  shape,
  globe,
  position
) {
  ...
  if (Array.isArray(shape.boundaries[0])) {
    ...
    for (var i = 0, lenPoly = locations.length; i < lenPoly; i++) {
      for (
        var j = 1, lenVertices = locations[i].length;
        j <= lenVertices;
        j++
      ) {
        ...
        pointOnEdge = this.closestPointOnSegment(pointA, pointB, pointPicked);
    ...
    if (nearestDistance < 20000) {
      ...
      if (nearestSegmentIndex == locations[nearestPolyIndex].length) {
        ...
        locations[nearestPolyIndex].splice(
          nearestSegmentIndex,
          0,
          nearestLocation
        );
...
export default SurfacePolygonEditorFragment;
