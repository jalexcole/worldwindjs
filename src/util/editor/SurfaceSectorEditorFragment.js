...
SurfaceSectorEditorFragment.prototype = Object.create(
...
SurfaceSectorEditorFragment.prototype.createShadowShape = function (shape) {
  return new SurfaceSector(
    new Sector(
      shape._boundaries[0].latitude,
      shape._boundaries[1].latitude,
      shape._boundaries[0].longitude,
...
// Internal use only.
SurfaceSectorEditorFragment.prototype.initializeControlElements = function (
  shape,
  controlPoints,
  shadowControlPoints,
  accessories,
  ...
  if (resizeControlPointAttributes) {
    this.createControlPoint(
      controlPoints,
      resizeControlPointAttributes,
      ShapeEditorConstants.MIN_CORNER
    );
    this.createControlPoint(
      controlPoints,
      resizeControlPointAttributes,
      ShapeEditorConstants.MAX_CORNER
    );
  }
};

// Internal use only.
SurfaceSectorEditorFragment.prototype.updateControlElements = function (
  shape,
  globe,
  controlPoints
) {
  if (controlPoints.length > 0) {
    controlPoints[0].position = new Location(
      shape._sector.minLatitude,
      shape._sector.minLongitude
    );
    controlPoints[1].position = new Location(
...
// Internal use only.
SurfaceSectorEditorFragment.prototype.reshape = function (
  shape,
  globe,
  controlPoint,
  currentPosition,
  ...
  if (controlPoint.userProperties.purpose === ShapeEditorConstants.MIN_CORNER) {
    ...
    controlPoint.userProperties.purpose === ShapeEditorConstants.MAX_CORNER
  ) {
    shape.sector = new Sector(
      shape._sector.minLatitude,
      currentPosition.latitude,
...
export default SurfaceSectorEditorFragment;
