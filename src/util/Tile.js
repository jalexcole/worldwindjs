...
class Tile {
   ...
   */
  static computeTileKey(levelNumber, row, column) {
   ...
   */
  static computeRow(delta, latitude) {
   ...
   */
  static computeColumn(delta, longitude) {
   ...
   * @returns {Number} The computed row number.
   */
  static computeLastRow(delta, maxLatitude) {
   ...
   * @returns {Number} The computed column number.
   */
  static computeLastColumn(delta, maxLongitude) {
  ...
  static computeSector(level, row, column) {
    ...
    var deltaLat = level.tileDelta.latitude, deltaLon = level.tileDelta.longitude, minLat = -90 + row * deltaLat, minLon = -180 + column * deltaLon, maxLat = minLat + deltaLat, maxLon = minLon + deltaLon;
  ...
  static createTilesForLevel(level, tileFactory, result) {
    ...
    for (var row = firstRow; row <= lastRow; row += 1) {
      ...
      for (var col = firstCol; col <= lastCol; col += 1) {
        ...
        var tileSector = new Sector(minLat, maxLat, minLon, maxLon), tile = tileFactory.createTile(tileSector, level, row, col);
        result.push(tile);
  ...
  size() {
    return (
      ...
      (4 + 32) + // nearest point
      8
    ); // extent timestamp and vertical exaggeration
  }
  /**
  ...
  subdivide(level, tileFactory) {
    ...
    var latMin = this.sector.minLatitude, latMax = this.sector.maxLatitude, latMid = this.sector.centroidLatitude(), lonMin = this.sector.minLongitude, lonMax = this.sector.maxLongitude, lonMid = this.sector.centroidLongitude(), subRow, subCol, childSector, children = [];
   ...
   * @throws {ArgumentError} If the specified tile factory or level is null or undefined.
   */
  subdivideToCache(level, tileFactory, cache) {
    if (!level) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
  ...
  mustSubdivide(dc, detailFactor) {
    ...
    var cellSize = dc.globe.equatorialRadius * this.texelSize, distance = this.sector.distanceTo(dc, dc.eyePoint), pixelSize = dc.pixelSizeAtDistance(distance);
   ...
   */
  update(dc) {
    var elevationTimestamp = dc.globe.elevationTimestamp(), verticalExaggeration = dc.verticalExaggeration, globeStateKey = dc.globeStateKey;
  ...
  doUpdate(dc) {
    ...
    // tile's sector.
    var globe = dc.globe, verticalExaggeration = dc.verticalExaggeration, extremes = globe.minAndMaxElevationsForSector(this.sector), minHeight = extremes[0] * verticalExaggeration, maxHeight = extremes[1] * verticalExaggeration;
