 ...
 * [WorldWind.BingMapsKey]{@link WorldWindConstants#BingMapsKey} is used. If that is null or undefined, the default
 ...
 * [WorldWind.BingMapsKey]{@link WorldWindConstants#BingMapsKey}.
 */
class BingImageryUrlBuilder {
  constructor(imagerySet, bingMapsKey) {
    ...
    if (!this.bingMapsKey) {
      this.bingMapsKey = WorldWindConstants.BingMapsKey;
    }
  ...
  urlForTile(tile, imageFormat) {
    ...
    var quadKey = this.quadKeyFromLevelRowColumn(
      tile.level.levelNumber,
      tile.row,
      tile.column
    ), url;
  ...
  // Intentionally not documented.
  quadKeyFromLevelRowColumn(levelNumber,
    row,
    column) {
    var digit, mask, key = "";
