...
var MeasurerUtils = {
  ...
  subdividePositions: function (
    ...
    for (var i = 1; i < positions.length; i++) {
      ...
      var arcLength =
        arcLengthRadians * globe.radiusAt(loc.latitude, loc.longitude);
      if (arcLength > maxLength) {
        ...
        for (var j = 1; j < steps; j++) {
          ...
          if (pathType === WorldWind.LINEAR) {
            ...
            destLatLon = Location.rhumbLocation(
              pos1,
              segmentAzimuth,
              s * segmentDistance,
              destLatLon
            ...
            destLatLon = Location.greatCircleLocation(
              pos1,
              segmentAzimuth,
              s * segmentDistance,
              destLatLon
          ...
          // Set elevation
          if (followTerrain) {
            elevation = globe.elevationAtLocation(
              destLatLon.latitude,
              destLatLon.longitude
            );
          } else {
          ...
          subdividedPositions.push(
            new Position(destLatLon.latitude, destLatLon.longitude, elevation)
          );
        }
      }
  ...
  addPosition: function (globe, positions, position, followTerrain) {
    ...
    positions.push(
      new Position(position.latitude, position.longitude, elevation)
    );
    return positions;
  },
