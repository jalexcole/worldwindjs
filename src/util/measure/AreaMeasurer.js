 ...
 * current pathType - WorldWindConstants.LINEAR, WorldWindConstants.RHUMB_LINE or WorldWindConstants.GREAT_CIRCLE.</p>
 *
 ...
 * @return {Number} area in square meters
 */
AreaMeasurer.prototype.computeSurfaceAreaSampling = function (
  globe,
  positions,
  pathType
) {
  ...
  this.subdividedPositions = MeasurerUtils.subdividePositions(
    globe,
    positions,
    true,
    pathType,
  ...
  var steps = Math.max(
    this.DEFAULT_AREA_SAMPLING_STEPS,
    this._areaTerrainSamplingSteps
  );
  ...
  var lonSteps = Math.round(
    (deltaLonRadians / stepsRadians) *
  ...
  for (var i = 0; i < latSteps; i++) {
    var lat =
      sector.minLatitude * Angle.DEGREES_TO_RADIANS + latStepRadians * i;
    // Compute this latitude row cells area
    var radius = globe.radiusAt(
      (lat + latStepRadians / 2) * Angle.RADIANS_TO_DEGREES,
      sector.centroidLongitude()
    );
    var cellWidth =
      lonStepRadians * radius * Math.cos(lat + latStepRadians / 2);
    var cellHeight = latStepRadians * radius;
    var cellArea = cellWidth * cellHeight;

    for (var j = 0; j < lonSteps; j++) {
      var lon =
        sector.minLongitude * Angle.DEGREES_TO_RADIANS + lonStepRadians * j;
      ...
      var isLocationInside = MeasurerUtils.isLocationInside(
        cellSector.centroid(this.scratchLocation),
      ...
      if (isLocationInside) {
        ...
        var tan = Math.tan(
          MeasurerUtils.angleBetweenVectors(this.vecZ, normalSW)
        );
 ...
 * @return {Number} area in square meters
 */
AreaMeasurer.prototype.computeProjectedAreaGeometry = function (
  globe,
  positions,
  pathType
) {
...
AreaMeasurer.prototype.computeTriangleProjectedArea = function (globe, verts) {
  ...
  var radius = globe.radiusAt(
    centerLat * Angle.RADIANS_TO_DEGREES,
    centerLon * Angle.RADIANS_TO_DEGREES
  );
