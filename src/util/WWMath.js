...
var WWMath = {
   ...
   */
  computeEllipsoidalGlobeIntersection: function (
    line,
    equatorialRadius,
    polarRadius,
    result
   ...
   */
  computeTriangleIntersection: function (
    line,
    vertex0,
    vertex1,
    vertex2,
  ...
  computeTriangleListIntersection: function (line, points, results) {
    ...
    for (var i = 0, len = points.length; i < len; i += 3) {
      if (
        WWMath.computeTriangleIntersection(
          line,
          points[i],
          points[i + 1],
          points[i + 2],
  ...
  computeIndexedTrianglesIntersection: function (
    line,
    points,
    indices,
    results
   ...
   */
  localCoordinateAxesAtPoint: function (
    origin,
    globe,
    xAxisResult,
    yAxisResult,
   ...
   * than one.
   */
  perspectiveNearDistanceForFarDistance: function (
    farDistance,
    farResolution,
    depthBits
  ) {
    ...
    return (
      farDistance /
      (maxDepthValue / (1 - farResolution / farDistance) - maxDepthValue + 1)
    );
  },
   ...
   * specified distance is negative.
   */
  perspectiveFrustumRectangle: function (
    viewportWidth,
    viewportHeight,
    distance
  ) {
  ...
  perspectivePixelSize: function (viewportWidth, viewportHeight, distance) {
    ...
    var frustumHeight = WWMath.perspectiveFrustumRectangle(
      viewportWidth,
      viewportHeight,
      distance
    ).height;
  ...
  gudermannianInverse: function (latitude) {
    return (
      Math.log(
        Math.tan(Math.PI / 4 + (latitude * Angle.DEGREES_TO_RADIANS) / 2)
      ) / Math.PI
    );
  },
