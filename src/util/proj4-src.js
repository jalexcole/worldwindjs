...
Parser.prototype.afterquote = function (char) {
  ...
  throw new Error(
    "havn't handled \"" + char + '" in afterquote yet, index ' + this.place
  );
};
...
Parser.prototype.number = function (char) {
  ...
  throw new Error(
    "havn't handled \"" + char + '" in number yet, index ' + this.place
  );
};
...
Parser.prototype.keyword = function (char) {
  ...
  throw new Error(
    "havn't handled \"" + char + '" in keyword yet, index ' + this.place
  );
};
Parser.prototype.neutral = function (char) {
  ...
  throw new Error(
    "havn't handled \"" + char + '" in neutral yet, index ' + this.place
  );
};
...
function transformer(from, to, coords) {
  ...
  if (Array.isArray(coords)) {
    ...
    if (coords.length > 2) {
      if (
        (typeof from.name !== "undefined" && from.name === "geocent") ||
        ...
        if (typeof transformedArray.z === "number") {
          ...
          return [transformedArray.x, transformedArray.y, coords[2]].concat(
            coords.splice(3)
          );
        }
      } else {
...
function init$3() {
  ...
  this.gtu[3] =
    np * (49561 / 161280 + n * (-179 / 168 + n * (6601661 / 7257600)));
...
function inverse$5(p) {
  ...
  for (var i = MAX_ITER$1; i > 0; --i) {
    lat =
      2 * Math.atan(num * srat(this.e * Math.sin(p.y), -0.5 * this.e)) -
      HALF_PI;
  ...
  if (this.sphere) {
    ...
    if (
      ...
      this.k0 =
        (0.5 *
          this.cons *
          msfnz(this.e, Math.sin(this.lat_ts), Math.cos(this.lat_ts))) /
        tsfnz(this.e, this.con * this.lat_ts, this.con * Math.sin(this.lat_ts));
    }
    this.ms1 = msfnz(this.e, this.sinlat0, this.coslat0);
    this.X0 =
      2 * Math.atan(this.ssfn_(this.lat0, this.sinlat0, this.e)) - HALF_PI;
...
function forward$6(p) {
  ...
  if (this.sphere) {
    ...
    A =
      (2 * this.k0) /
      (1 + this.sinlat0 * sinlat + this.coslat0 * coslat * Math.cos(dlon));
    p.x = this.a * A * coslat * Math.sin(dlon) + this.x0;
    p.y =
      this.a *
        A *
        (this.coslat0 * sinlat - this.sinlat0 * coslat * Math.cos(dlon)) +
      this.y0;
    return p;
  } else {
    ...
    if (Math.abs(this.coslat0) <= EPSLN) {
      ...
      A =
        ...
        (this.cosX0 *
          (1 + this.sinX0 * sinX + this.cosX0 * cosX * Math.cos(dlon)));
      p.y =
        A * (this.cosX0 * sinX - this.sinX0 * cosX * Math.cos(dlon)) + this.y0;
    }
...
function inverse$6(p) {
  ...
  if (this.sphere) {
    ...
    lat = Math.asin(
      Math.cos(c) * this.sinlat0 + (p.y * Math.sin(c) * this.coslat0) / rh
    );
    if (Math.abs(this.coslat0) < EPSLN) {
      if (this.lat0 > 0) {
      ...
      lon = adjust_lon(
        this.long0 +
          Math.atan2(
            p.x * Math.sin(c),
            rh * this.coslat0 * Math.cos(c) - p.y * this.sinlat0 * Math.sin(c)
          )
      );
    }
    p.x = lon;
    ...
    if (Math.abs(this.coslat0) <= EPSLN) {
      ...
      lon =
        this.con *
        adjust_lon(this.con * this.long0 + Math.atan2(p.x, -1 * p.y));
    } else {
      ...
      if (rh <= EPSLN) {
        ...
        Chi = Math.asin(
          Math.cos(ce) * this.sinX0 + (p.y * Math.sin(ce) * this.cosX0) / rh
        );
        lon = adjust_lon(
          this.long0 +
            Math.atan2(
              p.x * Math.sin(ce),
              rh * this.cosX0 * Math.cos(ce) - p.y * this.sinX0 * Math.sin(ce)
            )
        );
      }
...
function forward$7(p) {
  ...
  p.y =
    (this.R / 2) * Math.log((1 + Math.sin(rotB)) / (1 - Math.sin(rotB))) +
    this.y0;
...
function inverse$7(p) {
  ...
  var I = Math.atan(
    Math.sin(rotI) /
      (Math.cos(this.b0) * Math.cos(rotI) - Math.sin(this.b0) * Math.tan(rotB))
  );
  ...
  while (Math.abs(phy - prevPhy) > 0.0000001) {
    ...
    S =
      ...
      this.e *
        Math.log(Math.tan(Math.PI / 4 + Math.asin(this.e * Math.sin(phy)) / 2));
    prevPhy = phy;
...
function init$9() {
  ...
  this.al =
    (this.a * this.bl * this.k0 * Math.sqrt(1 - this.es)) / (1 - con * con);
  ...
  if (!isNaN(this.longc)) {
    ...
    this.long0 =
      ...
      Math.atan((jl * Math.tan(0.5 * this.bl * dlon12)) / pl) / this.bl;
  ...
  if (Math.abs(Math.abs(lat) - HALF_PI) <= EPSLN) {
    ...
    if (Math.abs(Math.cos(this.bl * dlon)) <= EPSLN) {
      ...
      us =
        (this.al *
          Math.atan2(
            sl * Math.cos(this.gamma0) + vl * Math.sin(this.gamma0),
...
function inverse$8(p) {
  ...
  if (Math.abs(up - 1) < EPSLN) {
    ...
    p.x = adjust_lon(
      this.long0 -
        Math.atan2(
          sp * Math.cos(this.gamma0) - vp * Math.sin(this.gamma0),
...
function inverse$11(p) {
  ...
  if (this.sphere) {
    ...
    lam =
      (dl * (1 - dsq * (tl1 / 3 + ((1 + 3 * tl1) * tl1 * dsq) / 15))) /
      Math.cos(phi1);
  }
...
function forward$17(p) {
  ...
  if (this.sphere) {
    if (Math.abs(lat) <= EPSLN) {
      ...
      y =
        this.a *
        (adjust_lat(lat - this.lat0) + (1 - Math.cos(el)) / Math.tan(lat));
    }
  } else {
    if (Math.abs(lat) <= EPSLN) {
      x = this.a * dlon;
  ...
  if (this.sphere) {
    if (Math.abs(y + this.a * this.lat0) <= EPSLN) {
      ...
      for (i = MAX_ITER$2; i; --i) {
        ...
        dphi =
          (-1 *
            (al * (phi * tanphi + 1) - phi - 0.5 * (phi * phi + bl) * tanphi)) /
          ((phi - al) / tanphi - 1);
        phi += dphi;
      ...
      lon = adjust_lon(
        this.long0 + Math.asin((x * Math.tan(phi)) / this.a) / Math.sin(lat)
      );
    }
  } else {
    if (Math.abs(y + this.ml0) <= EPSLN) {
      ...
      for (i = MAX_ITER$2; i; --i) {
        ...
        dphi =
          ...
          ((this.es * Math.sin(2 * phi) * (ma * ma + bl - 2 * al * ma)) /
            (4 * cl) +
  ...
  if (this.sphere) {
    if (Math.abs(this.sin_p12 - 1) <= EPSLN) {
      ...
      p.y =
        ...
        this.a *
          kp *
          (this.cos_p12 * sinphi - this.sin_p12 * cosphi * Math.cos(dlon));
      return p;
    }
  } else {
    e0 = e0fn(this.es);
    ...
    if (Math.abs(this.sin_p12 - 1) <= EPSLN) {
      ...
      psi = Math.atan(
        (1 - this.es) * tanphi + (this.es * Nl1 * this.sin_p12) / (Nl * cosphi)
      );
      Az = Math.atan2(
        Math.sin(dlon),
      ...
      if (Az === 0) {
        s = Math.asin(
          this.cos_p12 * Math.sin(psi) - this.sin_p12 * Math.cos(psi)
        );
...
function inverse$24(p) {
  ...
  if (this.sphere) {
    ...
    if (Math.abs(this.sin_p12 - 1) <= EPSLN) {
      ...
      A = (-tmp * tmp) / (1 - this.es);
      B =
        (3 * this.es * (1 - A) * this.sin_p12 * this.cos_p12 * cosAz) /
        (1 - this.es);
      D = rh / N1;
      Ee =
        D -
      ...
      psi = Math.asin(
        this.sin_p12 * Math.cos(Ee) + this.cos_p12 * Math.sin(Ee) * cosAz
      );
      lon = adjust_lon(
        this.long0 + Math.asin((Math.sin(Az) * Math.sin(Ee)) / Math.cos(psi))
      );
      lat = Math.atan(
        ((1 - (this.es * F * this.sin_p12) / Math.sin(psi)) * Math.tan(psi)) /
          (1 - this.es)
      );
      p.x = lon;
      p.y = lat;
...
function forward$26(p) {
  ...
  mu = Math.atan(
    (12 / SPI) *
      (theta + Math.acos(Math.sin(theta) * Math.cos(FORTPI)) - HALF_PI)
  );
  t = Math.sqrt(
    (1 - Math.cos(phi)) /
