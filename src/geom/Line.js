...
class Line {
   ...
   */
  static fromSegment(pointA, pointB) {
    if (!pointA || !pointB) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    var origin = new Vec3(pointA[0], pointA[1], pointA[2]), direction = new Vec3(
      pointB[0] - pointA[0],
      pointB[1] - pointA[1],
      pointB[2] - pointA[2]
    );
  ...
  equals(otherLine) {
    if (otherLine) {
      return (
        this.origin.equals(otherLine.origin) &&
        this.direction.equals(otherLine.direction)
      );
    }
