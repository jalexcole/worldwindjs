...
class BoundingBox {
  ...
  // Internal use only. Intentionally not documented.
  scratchElevations = new Float64Array(9);
  scratchPoints = new Float64Array(3 * this.scratchElevations.length);
  constructor() {}
  /**
  ...
  getCorners() {
    ...
    for (i = 0; i < 4; i++) {
      corners.push(
        new Vec3(this.topCenter[0], this.topCenter[1], this.topCenter[2])
      );
    }
   ...
   */
  setToSector(sector, globe, minElevation, maxElevation) {
    if (!sector) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    if (sector.deltaLongitude() > 180) {
      globe.computePointFromPosition(
        sector.centroidLatitude(),
        sector.centroidLongitude() + 90,
        maxElevation,
        this.tmp1
      );
      globe.computePointFromPosition(
  ...
  effectiveRadius(plane) {
    ...
    return (
      0.5 *
      (WWMath.fabs(this.r.dot(n)) +
        WWMath.fabs(this.s.dot(n)) +
        WWMath.fabs(this.t.dot(n)))
    );
  }
  /**
  ...
  intersectionPoint(plane) {
    var n = plane.normal,
      effectiveRadius =
        0.5 * (Math.abs(this.s.dot(n)) + Math.abs(this.t.dot(n)));

    return this.intersectsAt(plane, effectiveRadius, this.tmp1, this.tmp2);
  }
  // Internal. Intentionally not documented.
  intersectsAt(plane, effRadius, endPoint1, endPoint2) {
  ...
  // Internal. Intentionally not documented.
  swapAxes(a, aExtremes, b, bExtremes) {
    a.swap(b);
  ...
  render(dc) {
    ...
    let matrix = this.scratchMatrix;
    let program = dc.findAndBindProgram(BasicProgram);
