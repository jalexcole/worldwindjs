 ...
 */
// import { V } from "vitest/dist/reporters-5f784f42.js";
 ...
 */
  static averageOfBuffer(points, result )  {
   ...
   * @throws {ArgumentError} If any of the specified vectors are null or undefined.
   */
  static computeTriangleNormal(a , b , c )  {
    if (!a || !b || !c) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
  ...
  static findThreeIndependentVertices(coords, stride) {
    ...
    let a = new Vec3(coords[0], coords[1], coords[2]),
      b = null,
      c = null,
      k = xstride;
    ...
    for (k += xstride; k < coords.length; k += xstride) {
      ...
      if (
        ...
        if (!Vec3.areColinear(a, b, c)) break;
      }
   ...
   */
  static computeBufferNormal(coords, stride)  {
    var vertices = Vec3.findThreeIndependentVertices(coords, stride);

    return vertices
      ? Vec3.computeTriangleNormal(vertices[0], vertices[1], vertices[2])
      : null;
  };
