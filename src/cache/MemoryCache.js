...
class MemoryCache {
  ...
  putEntry(key, entry, size) {
    ...
    var existing = this.entries[key], cacheEntry;
  ...
  addCacheListener(listener) {
    ...
    if (typeof listener.entryRemoved != "function" ||
      typeof listener.removalError != "function") {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
          "MemoryCache",
  ...
  makeSpace(spaceRequired) {
    var sortedEntries = [], now = Date.now();
    ...
    sortedEntries.sort(function (a, b) {
      var aAge = (now - a.lastUsed) * a.agingFactor, bAge = (now - b.lastUsed) * b.agingFactor;
      return bAge - aAge;
    });

    for (var i = 0, len = sortedEntries.length; i < len; i++) {
      if (this.usedCapacity > this._lowWater ||
        this.freeCapacity < spaceRequired) {
