...
GpuResourceCache.prototype.putResource = function (key, resource, size) {
  ...
  this.entries.putEntry(
    key instanceof ImageSource ? key.key : key,
    entry,
    size
  );
};
...
GpuResourceCache.prototype.resourceForKey = function (key) {
  var entry =
    key instanceof ImageSource
      ? this.entries.entryForKey(key.key)
      : this.entries.entryForKey(key);
 ...
 */
GpuResourceCache.prototype.setResourceAgingFactor = function (
  key,
  agingFactor
) {
 ...
 */
GpuResourceCache.prototype.retrieveTexture = function (
  gl,
  imageSource,
  wrapMode,
  initialWidth,
  ...
  if (
    this.currentRetrievals[imageSource] ||
    this.absentResourceList.isResourceAbsent(imageSource)
  ) {
    return null;
  }
