...
class WmsLayerCapabilities {
  constructor(layerElement, parentNode) {
    if (!layerElement) {
  ...
  static accumulate(layer, propertyName, accumulation) {
  ...
  static replace(layer, propertyName) {
  ...
  static assembleStyle(styleElement) {
    var result = {};
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (childElement.localName === "Name") {
        ...
        result.legendUrls.push(
          WmsLayerCapabilities.assembleLegendUrl(childElement)
        );
  ...
  static assembleGeographicBoundingBox(bboxElement) {
    var result = {};
  ...
  static assembleLatLonBoundingBox(bboxElement) {
    // WMS 1.1.1
    var result = {};
  ...
  static assembleBoundingBox(bboxElement) {
    var result = {};
  ...
  static assembleDimension(dimensionElement) {
    var result = {};
    ...
    if (result.multipleValues) {
      result.multipleValues =
        result.multipleValues === "true" || result.multipleValues === "1";
    }
    result.nearestValue = dimensionElement.getAttribute("nearestValue");
    if (result.nearestValue) {
      result.nearestValue =
        result.nearestValue === "true" || result.nearestValue === "1";
    }
  ...
  static assembleAttribution(attributionElement) {
    var result = {};
  ...
  static assembleAuthorityUrl(urlElement) {
    var result = {};
  ...
  static assembleIdentifier(identifierElement) {
    var result = {};
  ...
  static assembleMetadataUrl(urlElement) {
    var result = {};
  ...
  static assembleLegendUrl(urlElement) {
    var result = {};

    result.width = WmsLayerCapabilities.getIntegerAttribute(urlElement, "width");
    result.height = WmsLayerCapabilities.getIntegerAttribute(
      urlElement,
      "height"
    );
  ...
  static assembleLogoUrl(urlElement) {
    var result = {};
  ...
  static getIntegerAttribute(element, attrName) {
  ...
  static getFloatAttribute(element, attrName) {
    var result = element.getAttribute(attrName);
  ...
  assembleLayer(layerElement) {
    ...
    for (c = 0; c < children.length; c++) {
      ...
      if (childElement.localName === "Layer") {
      ...
      } else if (childElement.localName === "SRS") {
        // WMS 1.1.1
        if (!this._srses) {
          this._srses = [];
        }
        ...
        this._geographicBoundingBox =
          WmsLayerCapabilities.assembleGeographicBoundingBox(childElement);
      } else if (childElement.localName === "LatLonBoundingBox") {
        // WMS 1.1.1
        this._geographicBoundingBox =
          WmsLayerCapabilities.assembleLatLonBoundingBox(childElement);
      } else if (childElement.localName === "BoundingBox") {
        if (!this._boundingBoxes) {
          this._boundingBoxes = [];
        }
        this._boundingBoxes.push(
          WmsLayerCapabilities.assembleBoundingBox(childElement)
        );
        ...
        this._dimensions.push(
          WmsLayerCapabilities.assembleDimension(childElement)
        );
      } else if (childElement.localName === "Extent") {
        // WMS 1.1.1
        if (!this._extents) {
          this._extents = [];
        }
        ...
        this._attribution =
          WmsLayerCapabilities.assembleAttribution(childElement);
      } else if (childElement.localName === "AuthorityURL") {
        if (!this._authorityUrls) {
          this._authorityUrls = [];
        }
        this._authorityUrls.push(
          WmsLayerCapabilities.assembleAuthorityUrl(childElement)
        );
        ...
        this.identifiers.push(
          WmsLayerCapabilities.assembleIdentifier(childElement)
        );
      } else if (childElement.localName === "MetadataURL") {
        if (!this.metadataUrls) {
          this.metadataUrls = [];
        }
        this.metadataUrls.push(
          WmsLayerCapabilities.assembleMetadataUrl(childElement)
        );
      ...
      } else if (childElement.localName === "ScaleHint") {
        // WMS 1.1.1
        this._scaleHint = {};
        this._scaleHint.min = WmsLayerCapabilities.getFloatAttribute(
          childElement,
          "min"
        );
        this._scaleHint.max = WmsLayerCapabilities.getFloatAttribute(
          childElement,
          "max"
        );
      }
...
Object.defineProperties(WmsLayerCapabilities.prototype, {
  ...
  dimensions: {
    get: function () {
      ...
      return accumulatedDimensions.length > 0
