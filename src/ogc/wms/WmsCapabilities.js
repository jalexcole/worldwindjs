...
class WmsCapabilities {
  ...
  accumulateNamedLayers(startLayers, namedLayersArray) {
  ...
  assembleContactInformation(element) {
    var contactInfo = {};
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (child.localName === "ContactPersonPrimary") {
        contactInfo.contactPersonPrimary =
          this.assembleContactPersonPrimary(child);
  ...
  assembleContactPersonPrimary(element) {
    var info = {};
  ...
  assembleContactAddress(element) {
    var address = {};
