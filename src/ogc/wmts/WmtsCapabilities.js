...
import WmtsLayerCapabilities from "./WmtsLayerCapabilities";
...
class WmtsCapabilities {
  ...
  static assembleTileMatrixSet(element) {
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (child.localName === "Identifier") {
        ...
        tileMatrixSet.tileMatrix.push(
          WmtsCapabilities.assembleTileMatrix(child)
        );
      }
    }
  ...
  static assembleTileMatrix(element) {
  ...
  static assembleServiceMetadataURL(element) {
    var result = {};
  ...
  assembleDocument(dom) {
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (child.localName === "ServiceIdentification") {
        ...
        this.serviceMetadataUrls.push(
          WmtsCapabilities.assembleServiceMetadataURL(child)
        );
      }
    }
  }
  assembleContents(element) {
    var contents = {};
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (child.localName === "Layer") {
        ...
        try {
          contents.tileMatrixSet.push(
            WmtsCapabilities.assembleTileMatrixSet(child)
          );
        } catch (e) {
          Logger.logMessage(
            Logger.LEVEL_SEVERE,
