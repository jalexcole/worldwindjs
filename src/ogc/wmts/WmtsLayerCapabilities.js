...
import OwsDescription from "../ows/OwsDescription";
 ...
 */
class WmtsLayerCapabilities extends OwsDescription {
  constructor(layerElement, capabilities) {
    super(layerElement);
    if (!layerElement) {
  ...
  static assembleBoundingBox(element) {
    ...
    if (element.localName === "WGS84BoundingBox") {
      result.getSector = function () {
        return new Sector(
          result.lowerCorner[1],
          result.upperCorner[1],
          result.lowerCorner[0],
          result.upperCorner[0]
        );
      };
    }
  ...
  static assembleDimension(element) {
    var result = new OwsDescription(element);
  ...
  static assembleMetadata(element) {
    // TODO
    var result = {};
  ...
  static assembleResourceUrl(element) {
    var result = {};
  ...
  static assembleLegendUrl(element) {
    var result = {};
  ...
  static assembleTileMatrixSetLink(element) {
    var result = {};
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (child.localName === "TileMatrixSet") {
        ...
        result.tileMatrixSetLimits =
          WmtsLayerCapabilities.assembleTileMatrixSetLimits(child);
      }
    }

    return result;
  }
  static assembleTileMatrixSetLimits(element) {
    var result = {};
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (child.localName === "TileMatrixLimits") {
        ...
        result.tileMatrixLimits.push(
          WmtsLayerCapabilities.assembleTileMatrixLimits(child)
        );
      }
    }
  ...
  static assembleTileMatrixLimits(element) {
    var result = {};
   ...
   * @returns {Array}
   */
  getLayerSupportedTileMatrixSets() {
    var tileMatrixSets = [];

    for (var i = 0, lenA = this.tileMatrixSetLink.length; i < lenA; i++) {
      var supportedTileMatrixSetIdentifier =
        this.tileMatrixSetLink[i].tileMatrixSet;
      for (
        var j = 0, lenB = this.capabilities.contents.tileMatrixSet.length;
        j < lenB;
        j++
      ) {
        var tileMatrixSetIdentifier =
          this.capabilities.contents.tileMatrixSet[j].identifier;
  ...
  assembleLayer(element) {
    ...
    for (var c = 0; c < children.length; c++) {
      ...
      if (child.localName === "Identifier") {
        ...
        this.wgs84BoundingBox =
          WmtsLayerCapabilities.assembleBoundingBox(child);
        ...
        this.tileMatrixSetLink.push(
          WmtsLayerCapabilities.assembleTileMatrixSetLink(child)
        );
      }
    }
  }
