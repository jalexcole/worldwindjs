...
import OwsDatasetSummary from "../ows/OwsDatasetSummary";
import OwsKeywords from "../ows/OwsKeywords";
import OwsOperationsMetadata from "../ows/OwsOperationsMetadata";
...
class WcsCapabilities {
  ...
  getCoverageBaseUrl() {
    if (this.version === "1.0.0") {
      ...
      return this.operationsMetadata.getOperationMetadataByName("GetCoverage")
  ...
  // Internal. Intentionally not documented.
  assembleDatasetAugment20x(element, coverage) {
    var children = element.children || element.childNodes;
    for (var c = 0; c < children.length; c++) {
      var child = children[c];
  ...
  // Internal. Intentionally not documented.
  assembleRequestCapabilities100(element) {
    var children = element.children || element.childNodes,
      request = {};
    for (var c = 0; c < children.length; c++) {
      var child = children[c];
  ...
  // Internal. Intentionally not documented.
  assembleHttp100(element, dcpType) {
    var children = element.children || element.childNodes;
  ...
  // Internal. Intentionally not documented.
  assembleMethod100(element, dcpType) {
    var children = element.children || element.childNodes;
  ...
  // Internal. Intentionally not documented.
  assembleOnlineResource100(element) {
    var children = element.children || element.childNodes;
  ...
  // Internal. Intentionally not documented.
  assembleLatLonBoundingBox(element) {
    var children = element.children || element.childNodes,
      boundingBox = {},
      previousValue,
      lonOne,
  ...
  // Internal. Intentionally not documented.
  assembleServiceMetadata(element) {
    var children = element.children || element.childNodes,
      serviceMetadata = {};
    for (var c = 0; c < children.length; c++) {
      var child = children[c];
      ...
      if (child.localName === "formatSupported") {
        serviceMetadata.formatsSupported =
          serviceMetadata.formatsSupported || [];
        serviceMetadata.formatsSupported.push(child.textContent);
      } else if (child.localName === "Extension") {
        serviceMetadata.extension =
          this.assembleServiceMetadataExtension(child);
      }
    }
  ...
  // Internal. Intentionally not documented.
  assembleServiceMetadataExtension(element) {
    var children = element.children || element.childNodes,
      len = children.length,
      extension = {};
    for (var c = 0; c < len; c++) {
