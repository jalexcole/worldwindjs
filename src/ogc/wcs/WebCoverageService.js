...
import WcsCapabilities from "./WcsCapabilities";
import WcsCoverage from "./WcsCoverage";
import WcsCoverageDescriptions from "./WcsCoverageDescriptions";
...
class WebCoverageService {
  ...
  retrieveCapabilities() {
    ...
    return (
      self
        .retrieveXml(self.buildCapabilitiesXmlRequest("2.0.1"))
  ...
  parseCoverages(xmlDom) {
    ...
    for (var i = 0; i < coverageCount; i++) {
      coverageId =
        this.coverageDescriptions.coverages[i].coverageId ||
      ...
      xhr.onerror = function () {
        reject(
          new Error(
            Logger.logMessage(
              Logger.LEVEL_WARNING,
              "XML retrieval failed: " + request.url
            )
          )
        );
      };
      xhr.ontimeout = function () {
        reject(
          new Error(
            Logger.logMessage(
              Logger.LEVEL_WARNING,
              "XML retrieval timed out: " + request.url
            )
          )
        );
      };
  ...
  // Internal use only
  buildCapabilitiesXmlRequest(version) {
    var capabilitiesElement = this.createBaseWcsElement(
      "GetCapabilities",
      version
    );
  ...
  // Internal use only
  buildDescribeCoverageXmlRequest() {
    ...
    if (version === "1.0.0") {
      ...
      requestUrl =
        this.capabilities.operationsMetadata.getOperationMetadataByName(
          "DescribeCoverage"
        ).dcp[0].getMethods[0].url;
    }

    for (var i = 0; i < coverageCount; i++) {
      if (version === "1.0.0") {
        coverageElement = document.createElementNS(
          WebCoverageService.WCS_XLMNS,
          "Coverage"
        );
        coverageElement.appendChild(
          document.createTextNode(this.capabilities.coverages[i].name)
        );
      } else if (version === "2.0.1" || version === "2.0.0") {
        coverageElement = document.createElementNS(
          WebCoverageService.WCS_2_XLMNS,
          "CoverageId"
        );
        coverageElement.appendChild(
          document.createTextNode(this.capabilities.coverages[i].coverageId)
        );
      }
  ...
  // Internal use only
  createBaseWcsElement(elementName, version) {
    var el;
