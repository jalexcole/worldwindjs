 ...
 */
class BasicWorldWindowController {
  ...
  gestureStateChanged(recognizer) {
    if (recognizer === this.primaryDragRecognizer ||
  ...
  handlePanOrDrag3D(recognizer) {
    var state = recognizer.state, tx = recognizer.translationX, ty = recognizer.translationY;
    ...
    if (state === WorldWind.BEGAN) {
      ...
      // to arc degrees.
      var lookAt = this.lookAt, canvas = this.wwd.canvas, globe = this.wwd.globe, globeRadius = WWMath.max(globe.equatorialRadius, globe.polarRadius), distance = WWMath.max(1, lookAt.range), metersPerPixel = WWMath.perspectivePixelSize(
        canvas.clientWidth,
        canvas.clientHeight,
        distance
      ), forwardMeters = (ty - this.lastPoint[1]) * metersPerPixel, sideMeters = -(tx - this.lastPoint[0]) * metersPerPixel, forwardDegrees = (forwardMeters / globeRadius) * Angle.RADIANS_TO_DEGREES, sideDegrees = (sideMeters / globeRadius) * Angle.RADIANS_TO_DEGREES;

      // Apply the change in latitude and longitude to the view, relative to the current heading.
      var sinHeading = Math.sin(lookAt.heading * Angle.DEGREES_TO_RADIANS), cosHeading = Math.cos(lookAt.heading * Angle.DEGREES_TO_RADIANS);
  ...
  handlePanOrDrag2D(recognizer) {
    var state = recognizer.state, x = recognizer.clientX, y = recognizer.clientY, tx = recognizer.translationX, ty = recognizer.translationY;
    ...
    if (state === WorldWind.BEGAN) {
      ...
      var x1 = this.lastPoint[0], y1 = this.lastPoint[1], x2 = this.beginPoint[0] + tx, y2 = this.beginPoint[1] + ty;

      this.lastPoint.set(x2, y2);

      var lookAt = this.lookAt, globe = this.wwd.globe, ray = this.wwd.rayThroughScreenPoint(this.wwd.canvasCoordinates(x1, y1)), point1 = new Vec3(0, 0, 0), point2 = new Vec3(0, 0, 0), origin = new Vec3(0, 0, 0);
  ...
  handleSecondaryDrag(recognizer) {
    ...
    if (state === WorldWind.BEGAN) {
      ...
      var lookAt = this.lookAt, headingDegrees = (180 * tx) / this.wwd.canvas.clientWidth, tiltDegrees = (90 * ty) / this.wwd.canvas.clientHeight;
  ...
  handlePinch(recognizer) {
    var state = recognizer.state, scale = recognizer.scale;
  ...
  handleRotation(recognizer) {
    var state = recognizer.state, rotation = recognizer.rotation;
  ...
  handleTilt(recognizer) {
    var state = recognizer.state, ty = recognizer.translationY;
...
BasicWorldWindowController.prototype = Object.create(
  WorldWindowController.prototype
);
