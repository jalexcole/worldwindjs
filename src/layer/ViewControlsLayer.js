 ...
 */
class ViewControlsLayer extends Layer{
  constructor(worldWindow) {
    ...
    this.panControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-pan-64x64.png"
    );
    this.zoomInControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-zoom-in-32x32.png"
    );
    this.zoomOutControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-zoom-out-32x32.png"
    );
    this.headingLeftControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-heading-left-32x32.png"
    );
    this.headingRightControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-heading-right-32x32.png"
    );
    this.tiltUpControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-pitch-up-32x32.png"
    );
    this.tiltDownControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-pitch-down-32x32.png"
    );
    this.exaggerationUpControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-elevation-up-32x32.png"
    );
    this.exaggerationDownControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-elevation-down-32x32.png"
    );
    this.fovNarrowControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-fov-narrow-32x32.png"
    );
    this.fovWideControl = new ScreenImage(
      screenOffset.clone(),
      imagePath + "view-fov-wide-32x32.png"
    );
  ...
  doRender(dc) {
    var controlPanelWidth = 0, controlPanelHeight = 64, panelOffset, screenOffset, x, y;
    ...
    screenOffset = this.placement.offsetForSize(
      dc.viewport.width,
      dc.viewport.height
    );
    panelOffset = this.alignment.offsetForSize(
      controlPanelWidth,
      controlPanelHeight
    );
    ...
    if (this.showExaggerationControl) {
      ...
      this.exaggerationUpControl.screenOffset.y =
  ...
  pickControl(e) {
    ...
    var x = pickPoint[0], y = this.wwd.canvas.height - pickPoint[1], control;
    ...
    for (var i = 0; i < this.controls.length; i++) {
      ...
      if (control.enabled) {
        if (x >= control.screenOffset.x &&
          x <= control.screenOffset.x + control.size &&
          y >= control.screenOffset.y &&
          y <= control.screenOffset.y + control.size) {
          return control;
        }
      }
    }
  ...
  determineOperation(e, topObject) {
    var operation = null;
  ...
  handlePan(e, control) {
    ...
    if (this.isPointerDown(e) || this.isPointerMove(e)) {
      ...
      var touch = e.changedTouches.item(0);
      this.currentEventPoint = this.wwd.canvasCoordinates(
        touch.clientX,
        touch.clientY
      );
    }
    ...
    if (this.isPointerDown(e) || this.isTouchStart(e)) {
      ...
      var setLookAtLocation = function () {
        if (thisLayer.activeControl) {
          var dx = thisLayer.panControlCenter[0] - thisLayer.currentEventPoint[0], dy = thisLayer.panControlCenter[1] -
            (thisLayer.wwd.viewport.height - thisLayer.currentEventPoint[1]), lookAt = thisLayer.lookAt, oldLat = lookAt.position.latitude, oldLon = lookAt.position.longitude,
            // Scale the increment by a constant and the relative distance of the eye to the surface.
            scale = thisLayer.panIncrement *
              (lookAt.range / thisLayer.wwd.globe.radiusAt(oldLat, oldLon)), heading = lookAt.heading + Math.atan2(dx, dy) * Angle.RADIANS_TO_DEGREES, distance = scale * Math.sqrt(dx * dx + dy * dy);
      ...
      var setExaggeration = function () {
        if (thisLayer.activeControl) {
          if (thisLayer.activeControl === thisLayer.exaggerationUpControl) {
            ...
            thisLayer.wwd.verticalExaggeration = Math.max(
              1,
              thisLayer.wwd.verticalExaggeration - thisLayer.exaggerationIncrement
            );
          }
      ...
      var setFov = function () {
        if (thisLayer.activeControl) {
          if (thisLayer.activeControl === thisLayer.fovWideControl) {
            thisLayer.wwd.camera.fieldOfView = Math.min(
              90,
              thisLayer.wwd.camera.fieldOfView + thisLayer.fieldOfViewIncrement
            );
          } else if (thisLayer.activeControl === thisLayer.fovNarrowControl) {
            thisLayer.wwd.camera.fieldOfView = Math.max(
              0,
              thisLayer.wwd.camera.fieldOfView - thisLayer.fieldOfViewIncrement
            );
          }
