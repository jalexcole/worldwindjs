 ...
 * values are used.
 */
class BMNGRestLayer extends Layer {
  constructor(serverAddress, pathToData, displayName, initialTime) {
    super(displayName || "Blue Marble time series");
  ...
  createSubLayer(layerName) {
    ...
    this.layers[layerName] = new RestTiledImageLayer(
      this.serverAddress,
      subLayerPath,
      this.displayName
    );
  ...
  nearestLayerName(time) {
    ...
    for (var i = 0; i < this.layerNames.length - 1; i++) {
      ...
      if (milliseconds >= leftTime && milliseconds <= rightTime) {
        ...
        return dLeft < dRight
          ? this.layerNames[i].month
          : this.layerNames[i + 1].month;
      }
    }
  }
}
