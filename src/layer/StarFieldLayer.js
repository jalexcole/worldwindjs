...
class StarFieldLayer extends Layer {
  ...
  loadCommonUniforms(dc) {
    ...
    var eyePosition = dc.globe.computePositionFromPoint(
      eyePoint[0],
      eyePoint[1],
      eyePoint[2],
      {}
  ...
  renderSun(dc) {
    ...
    if (this.sunSize > this._MAX_GL_POINT_SIZE) {
      Logger.log(
        ...
        "StarFieldLayer - sunSize is to big, max size allowed is: " +
        this._MAX_GL_POINT_SIZE
      );
    }

    var sunCelestialLocation = SunPosition.getAsCelestialLocation(
      this.time || new Date()
    );
  ...
  fetchStarData() {
    ...
    xhr.onload = function () {
      if (this.status >= 200 && this.status < 300) {
        try {
          ...
          Logger.log(
            Logger.LEVEL_SEVERE,
            "StarFieldLayer unable to parse JSON for star data " + e.toString()
          );
        }
      } else {
        Logger.log(
          Logger.LEVEL_SEVERE,
          "StarFieldLayer unable to fetch star data. Status: " +
          this.status +
          " " +
          this.statusText
        );
    ...
    xhr.ontimeout = function () {
      Logger.log(
        Logger.LEVEL_SEVERE,
        "StarFieldLayer fetch star data has timeout"
      );
      self._loadStarted = false;
    };
  ...
  parseStarsMetadata(metadata) {
    var raIndex = -1, decIndex = -1, magIndex = -1;
