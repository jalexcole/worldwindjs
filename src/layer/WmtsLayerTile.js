...
class WmtsLayerTile {
  constructor(sector, tileMatrix, row, column, imagePath) {
    this.sector = sector;
    ...
    this.texelSize =
      (sector.deltaLatitude() * Angle.DEGREES_TO_RADIANS) / tileMatrix.tileHeight;

    this.tileKey =
      tileMatrix.levelNumber.toString() +
      "." +
      row.toString() +
      "." +
      column.toString();
  ...
  distanceTo(vector) {
    ...
    var px = vector[0], py = vector[1], pz = vector[2], dx, dy, dz, points = this.samplePoints, distance = Number.POSITIVE_INFINITY;
  ...
  subdivide(tileMatrix, tileFactory) {
    if (!tileMatrix) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    var subRow, subCol, children = [], subFactorLat = tileMatrix.matrixHeight / this.tileMatrix.matrixHeight, subFactorLon = tileMatrix.matrixWidth / this.tileMatrix.matrixWidth;
  ...
  mustSubdivide(dc, detailFactor) {
    var cellSize = dc.globe.equatorialRadius * this.texelSize, distance = this.distanceTo(dc.eyePoint), pixelSize = dc.pixelSizeAtDistance(distance);
  ...
  doUpdate(dc) {
    ...
    globe.computePointsForGrid(
      this.sector,
      3,
      3,
      this.sampleElevations,
      Vec3.ZERO,
      this.samplePoints
    );
