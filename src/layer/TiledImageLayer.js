 ...
 *
 */
class TiledImageLayer extends Layer {
  constructor(displayName,
    ...
    tileWidth,
    tileHeight) {
    super(displayName || "Tiled Image Layer");
    ...
    if (this.currentTilesInvalid ||
      ...
      if (dc.surfaceOpacity >= 1 && this.opacity >= 1) {
        ...
        for (var j = 0; j < this.currentTiles.length; j++) {
          this.previousTiles[this.currentTiles[j].imagePath] =
            this.currentTiles[j];
        }
  ...
  isLayerInView(dc) {
    return (
      dc.terrain &&
      dc.terrain.sector &&
      dc.terrain.sector.intersects(this.levels.sector)
    );
  }
  // Documented in superclass.
  ...
  addTileOrDescendants(dc, tile) {
    ...
    try {
      ...
      var nextLevel = this.levels.level(tile.level.levelNumber + 1), subTiles = tile.subdivideToCache(nextLevel, this, this.tileCache);
