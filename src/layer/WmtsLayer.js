...
WmtsLayer.isTileSubdivisionCompatible = function (tileMatrixSet) {
  ...
  var matrixHeightRatio,
    matrixWidthRatio,
    tileMatrix,
    previousTileMatrix = tileMatrixSet.tileMatrix[0];
 ...
 */
WmtsLayer.formLayerConfiguration = function (
  ...
  // Configure URL
  if (
    wmtsLayerCapabilities.resourceUrl &&
  ...
  // set negotiation.
  var supportedTileMatrixSets =
    wmtsLayerCapabilities.getLayerSupportedTileMatrixSets();
  ...
  if (matrixSet) {
    for (var i = 0, len = supportedTileMatrixSets.length; i < len; i++) {
      if (
        supportedTileMatrixSets[i].identifier === matrixSet &&
...
WmtsLayer.prototype.addTileOrDescendants = function (dc, tile) {
  ...
  try {
    if (
      this.isTileTextureInMemory(dc, tile) ||
    ...
    var nextLevel =
        this.tileMatrixSet.tileMatrix[tile.tileMatrix.levelNumber + 1],
    ...
    for (var i = 0, len = subTiles.length; i < len; i++) {
      ...
      if (
        this.sector.intersects(child.sector) &&
...
WmtsLayer.prototype.computeTileMatrixValues3857 = function (tileMatrix) {
  var pixelSpan = tileMatrix.scaleDenominator * 0.28e-3,
    ...
    tileMatrixMaxX =
      tileMatrix.topLeftCorner[0] + tileSpanX * tileMatrix.matrixWidth,
    tileMatrixMinY =
      tileMatrix.topLeftCorner[1] - tileSpanY * tileMatrix.matrixHeight,
