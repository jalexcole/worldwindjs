...
class WmtsLayer extends Layer{
  constructor(config, timeString) {
    ...
    var supportedCrs = WmtsLayer.isEpsg3857Crs(crs) ||
      WmtsLayer.isEpsg4326Crs(crs) ||
      WmtsLayer.isOGCCrs84(crs);
    if (!supportedCrs) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    this.cachePath =
      (this.resourceUrl || this.serviceUrl) +
      this.layerIdentifier +
      this.styleIdentifier +
      this.tileMatrixSet.identifier;
    if (timeString) {
   ...
   */
  static isTileSubdivisionCompatible(tileMatrixSet) {
    if (!tileMatrixSet ||
      !tileMatrixSet.tileMatrix ||
      tileMatrixSet.tileMatrix.length < 1) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
          "WmtsLayer",
          "isTileSubdivisionCompatible",
    ...
    for (var i = 1, len = tileMatrixSet.tileMatrix.length; i < len; i++) {
      ...
      matrixHeightRatio =
        tileMatrix.matrixHeight / previousTileMatrix.matrixHeight;
  ...
  static createTileMatrixSet(params) {
    ...
    if (!(
      WmtsLayer.isEpsg4326Crs(params.projection) ||
      WmtsLayer.isOGCCrs84(params.projection) ||
      WmtsLayer.isEpsg3857Crs(params.projection)
    )) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
    ...
    var tileMatrixSet = [], scale;

    // Construct the tileMatrixSet
    for (var i = 0; i < params.resolutions.length; i++) {
      // Compute the scaleDenominator
      if (WmtsLayer.isEpsg4326Crs(params.projection) ||
        WmtsLayer.isOGCCrs84(params.projection)) {
        scale =
          (params.resolutions[i] * 6378137.0 * 2.0 * Math.PI) / 360 / 0.00028;
      ...
      var matrixWidth = Math.ceil(
        (params.extent[2] - params.extent[0] - 0.01 * unitWidth) / unitWidth
      );
      var matrixHeight = Math.ceil(
        (params.extent[3] - params.extent[1] - 0.01 * unitHeight) / unitHeight
      );
  ...
  static formLayerConfiguration(wmtsLayerCapabilities,
    ...
    // Configure URL
    if (wmtsLayerCapabilities.resourceUrl &&
      wmtsLayerCapabilities.resourceUrl.length >= 1) {
      ...
      // resource-oriented interface not supported, so use KVP interface
    ...
    if (matrixSet) {
      for (var i = 0, len = supportedTileMatrixSets.length; i < len; i++) {
        if (supportedTileMatrixSets[i].identifier === matrixSet &&
          WmtsLayer.isTileSubdivisionCompatible(supportedTileMatrixSets[i])) {
  ...
  static isEpsg4326Crs(crs) {
    return crs.indexOf("EPSG") >= 0 && crs.indexOf("4326") >= 0;
  }
  static isEpsg3857Crs(crs) {
    return (
      crs.indexOf("EPSG") >= 0 &&
      (crs.indexOf("3857") >= 0 || crs.indexOf("900913") >= 0)
    ); // 900913 is google's 3857 alias
  }
  static isOGCCrs84(crs) {
  ...
  isLayerInView(dc) {
    return (
      dc.terrain && dc.terrain.sector && dc.terrain.sector.intersects(this.sector)
    );
  }
  ...
  addTileOrDescendants(dc, tile) {
    ...
    try {
      if (this.isTileTextureInMemory(dc, tile) ||
        tile.tileMatrix.levelNumber === 0) {
        ancestorTile = this.currentAncestorTile;
        this.currentAncestorTile = tile;
      }
      var nextLevel = this.tileMatrixSet.tileMatrix[tile.tileMatrix.levelNumber + 1], subTiles = tile.subdivideToCache(nextLevel, this, this.tileCache);
      ...
      for (var i = 0, len = subTiles.length; i < len; i++) {
        ...
        if (this.sector.intersects(child.sector) &&
  ...
  isTextureExpired(texture) {
    return (
      this.expiration &&
      texture.creationTime.getTime() <= this.expiration.getTime()
    );
  }
  isTileTextureInMemory(dc, tile) {
    return dc.gpuResourceCache.containsResource(tile.imagePath);
  }
  tileMeetsRenderingCriteria(dc, tile) {
    ...
    return (
      tile.tileMatrix.levelNumber === this.tileMatrixSet.tileMatrix.length - 1 ||
      !tile.mustSubdivide(dc, s)
    );
  }
  retrieveTileImage(dc, tile) {
    if (this.currentRetrievals.indexOf(tile.imagePath) < 0) {
      ...
      var url = this.resourceUrlForTile(tile, this.imageFormat), image = new Image(), imagePath = tile.imagePath, cache = dc.gpuResourceCache, canvas = dc.currentGlContext.canvas, layer = this;
  ...
  createTile(tileMatrix, row, column) {
  ...
  createTileCrs84(tileMatrix, row, column) {
    var tileDeltaLat = this.sector.deltaLatitude() / tileMatrix.matrixHeight, tileDeltaLon = this.sector.deltaLongitude() / tileMatrix.matrixWidth, maxLat = tileMatrix.topLeftCorner[1] - row * tileDeltaLat, minLat = maxLat - tileDeltaLat, minLon = tileMatrix.topLeftCorner[0] + tileDeltaLon * column, maxLon = minLon + tileDeltaLon;

    var sector = new Sector(minLat, maxLat, minLon, maxLon);

    return this.makeTile(sector, tileMatrix, row, column);
  }
  createTile4326(tileMatrix, row, column) {
    var tileDeltaLat = this.sector.deltaLatitude() / tileMatrix.matrixHeight, tileDeltaLon = this.sector.deltaLongitude() / tileMatrix.matrixWidth, maxLat = tileMatrix.topLeftCorner[0] - row * tileDeltaLat, minLat = maxLat - tileDeltaLat, minLon = tileMatrix.topLeftCorner[1] + tileDeltaLon * column, maxLon = minLon + tileDeltaLon;

    var sector = new Sector(minLat, maxLat, minLon, maxLon);

    return this.makeTile(sector, tileMatrix, row, column);
  }
  createTile3857(tileMatrix, row, column) {
    if (!tileMatrix.mapWidth) {
      this.computeTileMatrixValues3857(tileMatrix);
    }

    var swX = WWMath.clamp(
      column * tileMatrix.tileWidth - 0.5,
      0,
      tileMatrix.mapWidth
    ), neY = WWMath.clamp(
      row * tileMatrix.tileHeight - 0.5,
      0,
      tileMatrix.mapHeight
    ), neX = WWMath.clamp(
      swX + tileMatrix.tileWidth + 0.5,
      0,
      tileMatrix.mapWidth
    ), swY = WWMath.clamp(
      neY + tileMatrix.tileHeight + 0.5,
      0,
      tileMatrix.mapHeight
  ...
  computeTileMatrixValues3857(tileMatrix) {
    var pixelSpan = tileMatrix.scaleDenominator * 0.28e-3, tileSpanX = tileMatrix.tileWidth * pixelSpan, tileSpanY = tileMatrix.tileHeight * pixelSpan, tileMatrixMaxX = tileMatrix.topLeftCorner[0] + tileSpanX * tileMatrix.matrixWidth, tileMatrixMinY = tileMatrix.topLeftCorner[1] - tileSpanY * tileMatrix.matrixHeight, bottomRightCorner = [tileMatrixMaxX, tileMatrixMinY], topLeftCorner = tileMatrix.topLeftCorner;
  ...
  makeTile(sector, tileMatrix, row, column) {
  ...
  createTexture3857(dc, tile, image) {
    if (!this.destCanvas) {
    ...
    var srcCanvas = dc.canvas2D, srcContext = dc.ctx2D, srcImageData, destCanvas = this.destCanvas, destContext = this.destContext, destImageData = destContext.createImageData(image.width, image.height), sector = tile.sector, tMin = WWMath.gudermannianInverse(sector.minLatitude), tMax = WWMath.gudermannianInverse(sector.maxLatitude), lat, g, srcRow, kSrc, kDest, sy, dy;
