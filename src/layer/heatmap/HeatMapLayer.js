 ...
 *  this layer.
 */
class HeatMapLayer extends TiledImageLayer{
  constructor(displayName, measuredLocations, numLevels) {
    super(
      displayName,
  ...
  filterGeographically(data, sector) {
    ...
    if (extraLongitudeBefore !== 0) {
      var beforeSector = new Sector(
        minLatitude,
        maxLatitude,
        180 - extraLongitudeBefore,
        180
      );
      this.gatherGeographical(
        data,
    ...
    if (extraLongitudeAfter !== 0) {
      var afterSector = new Sector(
        minLatitude,
        maxLatitude,
        -180,
        -180 + extraLongitudeAfter
      );
      this.gatherGeographical(
        data,
        result,
        ...
        data[lat][lon].forEach(function (element) {
          if (sector.containsLocation(element.latitude, element.longitude)) {
            if (!adaptLongitude) {
              ...
              result.push(
                new MeasuredLocation(
                  element.latitude,
                  adaptLongitude + element.longitude,
                  element.measure
                )
              );
            }
      ...
      if (data.length >= scale.length) {
        scale.forEach(function (color, index) {
          if (index === 0) {
            ...
            gradient[data[Math.floor((index / scale.length) * data.length)].measure / max] = color;
          }
        });
      } else {
        scale.forEach(function (color, index) {
  ...
  retrieveTileImage(dc, tile, suppressRedraw) {
    if (this.currentRetrievals.indexOf(tile.imagePath) < 0) {
      ...
      var imagePath = tile.imagePath, cache = dc.gpuResourceCache, layer = this, radius = this.radius;

      var extended = this.calculateExtendedSector(
        tile.sector,
        2 * (radius / this.tileWidth),
        2 * (radius / this.tileHeight)
      );
      var extendedWidth = Math.ceil(
        extended.extensionFactorWidth * this.tileWidth
      );
      var extendedHeight = Math.ceil(
        extended.extensionFactorHeight * this.tileHeight
      );
  ...
  calculateExtendedSector(sector,
    extensionFactorWidth,
    extensionFactorHeight) {
   ...
   */
  createHeatMapTile(data, options) {
