 ...
 * @augments Layer
 */
class AtmosphereLayer extends Layer{
  constructor(nightImageSource) {
    super("Atmosphere");
  ...
  applySkyVertices(dc) {
    var gl = dc.currentGlContext, program = dc.currentProgram, skyData = this._skyData, skyPoints, vboId;
    ...
    if (!vboId) {
      skyPoints = this.assembleVertexPoints(
        dc,
        this._skyHeight,
        this._skyWidth,
        program.getAltitude()
  ...
  // Internal. Intentionally not documented.
  applySkyIndices(dc) {
    var gl = dc.currentGlContext, skyData = this._skyData, skyIndices, vboId;
  ...
  // Internal. Intentionally not documented.
  drawSky(dc) {
    var gl = dc.currentGlContext, program = dc.findAndBindProgram(SkyProgram);
  ...
  // Internal. Intentionally not documented.
  drawGround(dc) {
    var gl = dc.currentGlContext, program = dc.findAndBindProgram(GroundProgram), terrain = dc.terrain, textureBound;
    ...
    if (this.nightImageSource && this.time !== null) {
      this._activeTexture = dc.gpuResourceCache.resourceForKey(
        this.nightImageSource
      );

      if (!this._activeTexture) {
        this._activeTexture = dc.gpuResourceCache.retrieveTexture(
          gl,
          this.nightImageSource
        );
      }
    ...
    for (var idx = 0, len = terrain.surfaceGeometry.length; idx < len; idx++) {
      ...
      if (textureBound) {
        ...
        this._texMatrix.multiplyByTileTransform(
          currentTile.sector,
      ...
      var fragMode = textureBound
        ? program.FRAGMODE_GROUND_PRIMARY_TEX_BLEND
        : program.FRAGMODE_GROUND_PRIMARY;
  ...
  // Internal. Intentionally not documented.
  assembleTriStripIndices(numLat, numLon) {
    var result = [];
    var vertex = 0;
  ...
  determineLightDirection(dc) {
    if (this.time !== null) {
      ...
      dc.globe.computePointFromLocation(
        sunLocation.latitude,
        sunLocation.longitude,
