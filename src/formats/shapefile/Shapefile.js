...
import ShapefileRecordMultiPoint from "./ShapefileRecordMultiPoint";
...
class Shapefile {
   ...
   */
  defaultShapeConfigurationCallback(attributes, record) {
    var configuration = {};
   ...
   *     [WorldWindConstants.RELATIVE_TO_GROUND]{@link WorldWind#RELATIVE_TO_GROUND} is used.</li>
  ...
  addRenderablesForPoints(layer) {
    ...
    for (var record = this.next(); !!record; record = this.next()) {
      var configuration = this.shapeConfigurationCallback(
        ...
        altitude =
          configuration && configuration.altitude ? configuration.altitude : 0;
      ...
      for (
        var part = 0, parts = record.numberOfParts;
        part < parts;
        part += 1
      ) {
        ...
        for (var idx = 0, len = points.length; idx < len; idx += 2) {
          ...
          placemark.altitudeMode =
            configuration.altitudeMode || WorldWindConstants.RELATIVE_TO_GROUND;
   ...
   * @throws {ArgumentError} If the specified layer is null or undefined.
   */
  addRenderablesForMultiPoints(layer) {
    if (!layer) {
      throw new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
          "Shapefile",
          "addRenderablesForMultiPoints",
          "missingLayer"
        )
      );
    }
   ...
   *     [WorldWindConstants.RELATIVE_TO_GROUND]{@link WorldWind#RELATIVE_TO_GROUND} is used.
  ...
  readHeader(buffer) {
    ...
    if (fileCode != Shapefile.FILE_CODE) {
      ...
      throw new Error(
        Logger.log(Logger.LEVEL_SEVERE, "Shapefile header is invalid")
      );
    }
    ...
    if (shapeType == null) {
      ...
      throw new Error(
        Logger.log(
          Logger.LEVEL_SEVERE,
          "Shapefile type is unsupported: " + type.toString()
        )
      );
    }
  ...
  // Intentionally not documented.
  readUnspecifiedBoundingRectangle(buffer) {
  ...
  // Intentionally not documented.
  readGeographicBoundingRectangle(buffer) {
    ...
    if (coords[1] > 90) {
      ...
      if (coords[0] > normalizedLat) coords[0] = normalizedLat;
    }
  ...
  // Intentionally not documented.
  readProjectedBoundingRectangle(buffer) {
    throw new NotYetImplementedError(
      Logger.log(
        Logger.LEVEL_SEVERE,
        "Shapefile.readProjectedBoundingRectangle() not yet implemented"
      )
    );
  ...
  // Intentionally not documented.
  readBoundingRectangleCoordinates(buffer) {
...
Object.defineProperties(Shapefile.prototype, {
   ...
   * <ul>
   *     <li>WorldWindConstants.POINT</li>
   *     <li>WorldWindConstants.MULTI_POINT</li>
   *     <li>WorldWindConstants.POLYLINE</li>
   *     <li>WorldWindConstants.POLYGON</li>
   * </ul>
