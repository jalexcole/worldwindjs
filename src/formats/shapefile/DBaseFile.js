...
import DBaseRecord from "./DBaseRecord";
...
class DBaseFile {
   ...
   */
  static isStringFilled(string, fillValue) {
    if (string.length <= 0) {
      return false;
    }

    for (var i = 0; i < string.length; i++) {
      if (string.charAt(i) != fillValue) return false;
    }
  ...
  getNumberOfFields() {
    return (
      (this.header.headerLength - 1 - DBaseFile.FIXED_HEADER_LENGTH) /
      DBaseFile.FIELD_DESCRIPTOR_LENGTH
    );
  }
  /**
  ...
  parse() {
    ...
    this.fields = this.readFieldDescriptors(
      this._buffer,
   ...
   */
  readFieldDescriptors(buffer, numFields) {
    var pos = buffer.position;
   ...
   */
  readNextRecord(buffer, recordNumber) {
   ...
   * @returns {String}
   */
  readNullTerminatedString(buffer, maxLength) {
    if (maxLength <= 0) {
      return 0;
    }
    ...
    if (this.isStringEmpty(string)) return "";
  ...
  isStringEmpty(string) {
    return (
      ...
      DBaseFile.isStringFilled(string, 0x2a)
    ); // Asterisk character.
  }
}
