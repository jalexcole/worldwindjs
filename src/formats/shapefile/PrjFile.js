...
class PrjFile {
   ...
   */
  decodeOGCCoordinateSystem(text) {
    if (!text) {
      new ArgumentError(
        Logger.logMessage(
          Logger.LEVEL_SEVERE,
          "PrjFile",
          "decodeOGCCoordinateSystem",
          "missingText"
        )
      );
    }
   ...
   */
  isUnknownCoordinateSystem() {
    return (
      !this.params ||
      this.params.coordinateSystem === PrjFile.COORDINATE_SYSTEM_UNKNOWN
    );
  }
  /**
  ...
  isGeographicCoordinateSystem() {
    return (
      !!this.params &&
      this.params.coordinateSystem === PrjFile.COORDINATE_SYSTEM_GEOGRAPHIC
    );
  }
  /**
  ...
  isProjectedCoordinateSystem() {
    return (
      !!this.params &&
      this.params.coordinateSystem === PrjFile.COORDINATE_SYSTEM_PROJECTED
    );
  }
}
...
PrjFile.UTM_NAME_WKT_PATTERN = new RegExp(
  ".*UTM.*ZONE.*?(\\d+).*?([\\w\\s]+).*?"
);
