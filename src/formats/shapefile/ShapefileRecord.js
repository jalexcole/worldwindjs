...
class ShapefileRecord {
   ...
   * @param {Number} array
   */
  static normalizeLocations(array) {
  ...
  readPolylineContents() {
    ...
    if (this._numberOfParts > 0 && this._numberOfPoints > 0) {
      ...
      for (var partNumber = 0; partNumber < this.numberOfParts; partNumber += 1) {
        ...
        this._parts[partNumber] = this._buffer.getDoubleArray(
          numPointsInPart * 2
        );
   ...
   */
  readOptionalMeasures(isPoint) {
    // Measure values are optional.
    if (this._buffer.hasRemaining() &&
      this._buffer.limit() - this._buffer.position >= this.numberOfPoints * 8) {
      if (isPoint) {
