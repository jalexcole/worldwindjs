...
GeoJSONParser.prototype.load = function (
  parserCompletionCallback,
  shapeConfigurationCallback,
  layer
) {
 ...
 */
GeoJSONParser.prototype.defaultShapeConfigurationCallback = function (
  geometry,
  properties
) {
  var configuration = {};
...
GeoJSONParser.prototype.handle = function (obj) {
  ...
  if (
    ...
    typeof this._parserCompletionCallback === "function"
  ) {
  ...
  if (this.geoJSONObject[GeoJSONConstants.FIELD_CRS]) {
    this._crs = new GeoJSONCRS(
      this.geoJSONObject[GeoJSONConstants.FIELD_CRS][
        GeoJSONConstants.FIELD_TYPE
      ],
      this.geoJSONObject[GeoJSONConstants.FIELD_CRS][
        GeoJSONConstants.FIELD_PROPERTIES
      ]
    );
...
GeoJSONParser.prototype.addRenderablesForGeoJSON = function (layer) {
  ...
  switch (this.geoJSONType) {
    case GeoJSONConstants.TYPE_FEATURE:
      ...
      this.addRenderablesForFeature(layer, feature);
      break;
    case GeoJSONConstants.TYPE_FEATURE_COLLECTION:
      ...
      this.addRenderablesForFeatureCollection(layer, featureCollection);
      break;
    case GeoJSONConstants.TYPE_GEOMETRY_COLLECTION:
      ...
      this.addRenderablesForGeometryCollection(layer, geometryCollection, null);
      break;
    default:
      this.addRenderablesForGeometry(layer, this.geoJSONObject, null);
      break;
  }
};
 ...
 */
GeoJSONParser.prototype.addRenderablesForGeometry = function (
  layer,
  geometry,
  properties
) {
 ...
 */
GeoJSONParser.prototype.addRenderablesForPoint = function (
  layer,
  geometry,
  properties
) {
  ...
  if (!this.crs || this.crs.isCRSSupported()) {
    ...
    var position = new Position(
      reprojectedCoordinate[1],
      reprojectedCoordinate[0],
      altitude
    );
    ...
    if (configuration && configuration.eyeDistanceScalingThreshold) {
      placemark.eyeDistanceScalingThreshold =
        configuration.eyeDistanceScalingThreshold;
    }
...
GeoJSONParser.prototype.addRenderablesForMultiPoint = function (
  ...
  if (!this.crs || this.crs.isCRSSupported()) {
    for (
      var pointIndex = 0, points = geometry.coordinates.length;
      pointIndex < points;
      pointIndex += 1
    ) {
      var longitude = geometry.coordinates[pointIndex][0],
        latitude = geometry.coordinates[pointIndex][1],
        altitude = geometry.coordinates[pointIndex][2]
          ? geometry.coordinates[pointIndex][2]
          : 0;
 ...
 * @throws {ArgumentError} If the specified geometry is null or undefined.
 */
GeoJSONParser.prototype.addRenderablesForLineString = function (
  layer,
  geometry,
  properties
) {
  ...
  if (!this.crs || this.crs.isCRSSupported()) {
    var positions = [];
    for (
      var pointsIndex = 0, points = geometry.coordinates;
 ...
 * @throws {ArgumentError} If the specified geometry is null or undefined.
 */
GeoJSONParser.prototype.addRenderablesForMultiLineString = function (
  layer,
  geometry,
  properties
) {
  if (!layer) {
    throw new ArgumentError(
      Logger.logMessage(
        Logger.LEVEL_SEVERE,
        "GeoJSON",
        "addRenderablesForMultiLineString",
        "missingLayer"
      )
    );
  }
  ...
  if (!this.crs || this.crs.isCRSSupported()) {
    for (
      var linesIndex = 0, lines = geometry.coordinates;
      ...
      for (
        var positionIndex = 0, points = lines[linesIndex];
        positionIndex < points.length;
        positionIndex++
      ) {
 ...
 * @throws {ArgumentError} If the specified geometry is null or undefined.
 */
GeoJSONParser.prototype.addRenderablesForPolygon = function (
  layer,
  geometry,
  properties
) {
  ...
  if (!this.crs || this.crs.isCRSSupported()) {
    var pBoundaries = [];
    for (
      var boundariesIndex = 0, boundaries = geometry.coordinates;
      boundariesIndex < boundaries.length;
      boundariesIndex++
    ) {
      var positions = [];

      for (
        var positionIndex = 0, points = boundaries[boundariesIndex];
        positionIndex < points.length;
        positionIndex++
      ) {
        var longitude = points[positionIndex][0],
 ...
 * @throws {ArgumentError} If the specified geometry is null or undefined.
 */
GeoJSONParser.prototype.addRenderablesForMultiPolygon = function (
  layer,
  geometry,
  properties
) {
  ...
  if (!this.crs || this.crs.isCRSSupported()) {
    for (
      var polygonsIndex = 0, polygons = geometry.coordinates;
      polygonsIndex < polygons.length;
      polygonsIndex++
    ) {
      var boundaries = [];
      for (
        var boundariesIndex = 0;
        boundariesIndex < polygons[polygonsIndex].length;
        boundariesIndex++
      ) {
        var positions = [];
        for (
          var positionIndex = 0,
            points = polygons[polygonsIndex][boundariesIndex];
          positionIndex < points.length;
          positionIndex++
        ) {
          var longitude = points[positionIndex][0],
 ...
 */
GeoJSONParser.prototype.addRenderablesForGeometryCollection = function (
  layer,
  geometryCollection,
  properties
) {
  if (!layer) {
    throw new ArgumentError(
      Logger.logMessage(
        Logger.LEVEL_SEVERE,
        "GeoJSON",
        "addRenderablesForGeometryCollection",
        "missingLayer"
      )
    );
  }
  ...
  for (
    var geometryIndex = 0, geometries = geometryCollection.geometries;
...
GeoJSONParser.prototype.addRenderablesForFeature = function (layer, feature) {
  ...
  if (feature.geometry.type === GeoJSONConstants.TYPE_GEOMETRY_COLLECTION) {
    ...
    this.addRenderablesForGeometry(layer, feature.geometry, feature.properties);
  }
};
 ...
 * @throws {ArgumentError} If the specified featureCollection is null or undefined.
 */
GeoJSONParser.prototype.addRenderablesForFeatureCollection = function (
  layer,
  featureCollection
) {
  if (!layer) {
    throw new ArgumentError(
      Logger.logMessage(
        Logger.LEVEL_SEVERE,
        "GeoJSON",
        "addRenderablesForFeatureCollection",
        "missingLayer"
      )
    );
  }
  ...
  if (featureCollection.features.length > 0) {
    for (
      ...
      var feature = new GeoJSONFeature(
        featureCollection.features[featureIndex][
          GeoJSONConstants.FIELD_GEOMETRY
        ],
        featureCollection.features[featureIndex][
          GeoJSONConstants.FIELD_PROPERTIES
        ],
 ...
 */
GeoJSONParser.prototype.getReprojectedIfRequired = function (
  latitude,
  longitude,
  crsObject
) {
...
GeoJSONParser.prototype.setProj4jsAliases = function () {
  Proj4.defs([
    ["urn:ogc:def:crs:EPSG::4326", Proj4.defs("EPSG:4326")],
    ["urn:ogc:def:crs:OGC:1.3:CRS84", Proj4.defs("EPSG:4326")],
    ["urn:ogc:def:crs:EPSG::3857", Proj4.defs("EPSG:3857")],
  ]);
};
