...
class GeoJSONParser {
   ...
   */
  defaultShapeConfigurationCallback(geometry,
    properties) {
    var configuration = {};
   ...
   */
  addRenderablesForGeometry(layer,
    geometry,
    properties) {
    if (!layer) {
      throw new ArgumentError(
  ...
  addRenderablesForPoint(layer,
    ...
    if (!this.crs || this.crs.isCRSSupported()) {
      var longitude = geometry.coordinates[0], latitude = geometry.coordinates[1], altitude = geometry.coordinates[2] ? geometry.coordinates[2] : 0;
   ...
   * @throws {ArgumentError} If the specified geometry is null or undefined.
   */
  addRenderablesForMultiPoint(layer,
    geometry,
    properties) {
    if (!layer) {
      throw new ArgumentError(
    ...
    if (!this.crs || this.crs.isCRSSupported()) {
      for (var pointIndex = 0, points = geometry.coordinates.length; pointIndex < points; pointIndex += 1) {
        var longitude = geometry.coordinates[pointIndex][0], latitude = geometry.coordinates[pointIndex][1], altitude = geometry.coordinates[pointIndex][2]
   ...
   * @throws {ArgumentError} If the specified geometry is null or undefined.
   */
  addRenderablesForLineString(layer,
    geometry,
    properties) {
    if (!layer) {
      throw new ArgumentError(
    ...
    if (!this.crs || this.crs.isCRSSupported()) {
      ...
      for (var pointsIndex = 0, points = geometry.coordinates; pointsIndex < points.length; pointsIndex++) {
        var longitude = points[pointsIndex][0], latitude = points[pointsIndex][1], altitude = points[pointsIndex][2] ? points[pointsIndex][2] : 0;
        var reprojectedCoordinate = this.getReprojectedIfRequired(
          latitude,
          longitude,
          this.crs
   ...
   * @throws {ArgumentError} If the specified geometry is null or undefined.
   */
  addRenderablesForMultiLineString(layer,
    geometry,
    properties) {
    if (!layer) {
      throw new ArgumentError(
    ...
    if (!this.crs || this.crs.isCRSSupported()) {
      for (var linesIndex = 0, lines = geometry.coordinates; linesIndex < lines.length; linesIndex++) {
        ...
        for (var positionIndex = 0, points = lines[linesIndex]; positionIndex < points.length; positionIndex++) {
          var longitude = points[positionIndex][0], latitude = points[positionIndex][1];
   ...
   * @throws {ArgumentError} If the specified geometry is null or undefined.
   */
  addRenderablesForPolygon(layer,
    geometry,
    properties) {
    if (!layer) {
      throw new ArgumentError(
    ...
    if (!this.crs || this.crs.isCRSSupported()) {
      ...
      for (var boundariesIndex = 0, boundaries = geometry.coordinates; boundariesIndex < boundaries.length; boundariesIndex++) {
        ...
        for (var positionIndex = 0, points = boundaries[boundariesIndex]; positionIndex < points.length; positionIndex++) {
          var longitude = points[positionIndex][0], latitude = points[positionIndex][1], altitude = points[positionIndex][2] ? points[positionIndex][2] : 0;
   ...
   * @throws {ArgumentError} If the specified geometry is null or undefined.
   */
  addRenderablesForMultiPolygon(layer,
    geometry,
    properties) {
    if (!layer) {
      throw new ArgumentError(
   ...
   */
  addRenderablesForGeometryCollection(layer,
    geometryCollection,
    properties) {
    if (!layer) {
      throw new ArgumentError(
   ...
   * @throws {ArgumentError} If the specified featureCollection is null or undefined.
   */
  addRenderablesForFeatureCollection(layer,
    featureCollection) {
    if (!layer) {
      throw new ArgumentError(
        Logger.logMessage(
   ...
   */
  getReprojectedIfRequired(latitude,
    longitude,
    crsObject) {
