...
KmlElementsFactoryCached.prototype.all = function (element) {
  ...
  if (elements && elements.length) {
    ...
    elements.forEach(function (pElement) {
      self.cache.add(
        self.cacheKey(parentNode, "All"),
        self.cacheKey(pElement.node),
        pElement
      );
    });
  }
...
KmlElementsFactoryCached.prototype.specific = function (element, options) {
  ...
  if (result && result.node) {
    this.cache.add(
      this.cacheKey(parentNode),
      this.cacheKey(result.node),
      result
    );
  } else if (result) {
...
KmlElementsFactoryCached.prototype.any = function (element, options) {
  ...
  if (result) {
    this.cache.add(
      self.cacheKey(parentNode),
      self.cacheKey(result.node),
      result
    );
  }
  return result;
