...
class KmlNodeTransformers {
   ...
   */
  static kmlObject(node, parent, controls) {
   ...
   */
  static linearRing(node, parent, controls) {
  ...
  static positions(node) {
    ...
    coordinates.forEach(function (pCoordinates) {
      ...
      positions.push(
        new Position(
          Number(pCoordinates[1]),
          Number(pCoordinates[0]),
          Number(pCoordinates[2] || 0)
        )
      );
    });
    return positions;
