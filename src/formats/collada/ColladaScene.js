 ...
 */
class ColladaScene extends Renderable{
    constructor(position, sceneData) {
        super();
        if (!position) {
    ...
    flattenModel() {
        ...
        this._entities.sort(function (a, b) {
            var va = (a.imageKey === null) ? "" : "" + a, vb = (b.imageKey === null) ? "" : "" + b;
     ...
     */
    computePointIntersections(globe, pointRay, results) {
        if (!globe) {
    ...
    // Internal. Intentionally not documented.
    applyMatrix(dc, hasLighting, hasTexture, nodeWorldMatrix, nodeNormalMatrix) {
    ...
    // Internal. Intentionally not documented.
    computeTransformationMatrix(globe) {
        if (!this._currentData.expired) {
            return;
        }
