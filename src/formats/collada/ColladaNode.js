...
class ColladaNode {
    ...
    parse(element, iNodes, parentWorldMatrix) {
        ...
        for (var i = 0; i < element.childNodes.length; i++) {
            ...
            switch (child.nodeName) {
                case "node":
                    var childNodes = new ColladaNode().parse(
                        child,
                        iNodes,
                        this.worldMatrix
                    );
     ...
     * @param {Matrix} parentWorldMatrix The transformation matrix of it's parent.
     */
    setNodeTransforms(element,
        parentWorldMatrix) {
        var matrix = Matrix.fromIdentity(), rotationMatrix = Matrix.fromIdentity(), translationMatrix = Matrix.fromIdentity(), scaleMatrix = Matrix.fromIdentity();
        ...
        for (var i = 0; i < element.childNodes.length; i++) {
            ...
            switch (child.nodeName) {
                ...
                case "rotate":
                    ...
                    rotationMatrix.multiplyByRotation(
                        values[0],
                        values[1],
                        values[2],
                        values[3]
                ...
                case "translate":
                    values = ColladaUtils.bufferDataFloat32(child);
                    translationMatrix.multiplyByTranslation(
                        values[0],
                        values[1],
                        values[2]
                    );
