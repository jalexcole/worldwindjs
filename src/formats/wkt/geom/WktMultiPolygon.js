 ...
 * @constructor
 */
class WktMultiPolygon extends WktObject{
  constructor() {
    super(WktType.SupportedGeometries.MULTI_POLYGON);
   ...
   * @private
   */
  rightParenthesis(options) {
    WktObject.prototype.rightParenthesis.call(this, options);

    // MultiPolygon object is distinguished by )),
    if (options.tokens[options.tokens.length - 1].type !=
      WktType.TokenType.RIGHT_PARENTHESIS) {
      this.addBoundaries();
      // MultiPolygon boundaries are distinguished by ),
    } else if (options.tokens[options.tokens.length - 1].type ==
      WktType.TokenType.RIGHT_PARENTHESIS &&
      options.tokens[options.tokens.length - 2].type !=
      WktType.TokenType.RIGHT_PARENTHESIS) {
      this.addObject();
    }
  }
