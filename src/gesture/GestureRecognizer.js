 ...
 * @param recognizer
 */
GestureRecognizer.prototype.recognizeSimultaneouslyWith = function (
  recognizer
) {
  if (!recognizer) {
    throw new ArgumentError(
 ...
 * @returns {Boolean}
 */
GestureRecognizer.prototype.canRecognizeSimultaneouslyWith = function (
  recognizer
) {
  var index = this._canRecognizeWith.indexOf(recognizer);
  return index != -1;
...
GestureRecognizer.prototype.tryToRecognize = function (newState) {
  ...
  if (
    GestureRecognizer.allRecognizers.some(this.canBePreventedByRecognizer, this)
  ) {
...
GestureRecognizer.prototype.canBePreventedByRecognizer = function (that) {
  return (
    this != that &&
    this.target == that.target &&
    that.state == WorldWind.RECOGNIZED &&
