 ...
 */
import SunPosition from "../../src/util/SunPosition.js";
...
describe("SunPosition Test", function () {
  describe("Compute the Sun's geographic location", function () {
    it("Computes the Sun's geographic location", function () {
      ...
      expectedLocations.forEach(function (expectedResult, i) {
        expect(expectedResult.latitude).toBeCloseTo(
          computedLocations[i].latitude
        );
        expect(expectedResult.longitude).toBeCloseTo(
          computedLocations[i].longitude
        );
      });
    });
  });
  ...
  describe("Computes the Sun's celestial location", function () {
    it("Computes the Sun's celestial location", function () {
      ...
      var expectedLocations = [
        ...
        {
          declination: -23.420379101540863,
          rightAscension: 267.75877297672787,
        },
        {
          declination: -23.430434209235372,
          rightAscension: 271.36570946223924,
        },
      ];
      var computedLocations = dates.map(function (date) {
      ...
      expectedLocations.forEach(function (expectedResult, i) {
        expect(expectedResult.declination).toBeCloseTo(
          computedLocations[i].declination
        );
        expect(expectedResult.rightAscension).toBeCloseTo(
          computedLocations[i].rightAscension
        );
      });
    });
  });
  ...
  describe("Convert celestial to geographic coordinates", function () {
    it("Convert celestial to geographic coordinates", function () {
      ...
      var geographicLocations = celestialLocations.map(function (
        celestialLocation
      ) {
      ...
      expectedGeographicLocations.forEach(function (expectedResult, i) {
        expect(expectedResult.latitude).toBeCloseTo(
          geographicLocations[i].latitude
        );
        expect(expectedResult.longitude).toBeCloseTo(
          geographicLocations[i].longitude
        );
      });
    });
  });
  ...
  describe("Compute julian date", function () {
    it("Compute julian date", function () {
      ...
      var expectedJulianDates = [
        2457755, 2457924.25, 2457926.75, 2457967.0833333335, 2458107.1666666665,
        2458110.4166666665,
      ];
