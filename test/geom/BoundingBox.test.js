...
describe("BoundingBox Tests", function () {
  it("Should have the basic bounding box properties.", function () {
    ...
    expect(boundingBox.scratchPoints).toEqual(
      new Float64Array(3 * boundingBox.scratchElevations.length)
    );
  });
  ...
  describe("Set to points method", function () {
    it("Sets this bounding box such that it minimally encloses a specified collection of points", function () {
      ...
      expect(boundingBox.setToPoints(samplePoints).radius).toEqual(
        1.0606601717798212
      );
    });
    ...
    it("Sets this bounding box such that it minimally encloses a specified collection of points", function () {
      ...
      expect(boundingBox.setToVec3Points(vecPoints).radius).toEqual(
        1.0606601717798212
      );
    });
  ...
  describe("Get corners method", function () {
    it("Creates a box, verifies correct corners retrieved", function () {
      ...
      var results = [
        ...
        3.368652, 15.397291, -19.430232, -8.270784, 17.184283, -16.194222,
        -3.368652, -15.397291, 19.430232, 8.270784, -17.184283, 16.194222,
        13.006629, 5.673621, 20.605725, 1.367193, 7.460613, 23.841735,
      ];
      var resultCount = 0;
  ...
  describe("Set to sector method", function () {
    it("Sets this bounding box to contain an entire globe", function () {
      ...
      var boundingBox = new BoundingBox().setToSector(
        sector,
        globe,
        minElevation,
        maxElevation
      ...
      expect(boundingBox.bottomCenter).toEqualVec3(
        new Vec3(-equatorialExtreme, 0, 0),
        1.0e-9
      );
      expect(boundingBox.topCenter).toEqualVec3(
        new Vec3(equatorialExtreme, 0, 0),
        1.0e-9
      );
      expect(boundingBox.r).toEqualVec3(
        new Vec3(equatorialExtreme * 2, 0, 0),
        1.0e-9
      );
      expect(boundingBox.s).toEqualVec3(
        new Vec3(0, 0, equatorialExtreme * 2),
        1.0e-6
      );
      expect(boundingBox.t).toEqualVec3(
        new Vec3(0, polarExtreme * 2, 0),
        1.0e-1
      );
    });
