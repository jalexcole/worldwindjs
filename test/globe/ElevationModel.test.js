...
import Tessellator from "../../src/globe/Tessellator";
import GebcoElevationCoverage from "../../src/globe/GebcoElevationCoverage";
import AsterV2ElevationCoverage from "../../src/globe/AsterV2ElevationCoverage";
import UsgsNedElevationCoverage from "../../src/globe/UsgsNedElevationCoverage";
...
describe("ElevationModel tests", function () {
  ...
  describe("Elevation result tests", function () {
    ...
    it("Prioritizes coverages correctly according to resolution", function () {
      ...
      for (var i = 0; i < ts.maximumSubdivisionDepth; i++) {
        ...
        var preferredIndex = em.preferredCoverageIndex(
          Sector.FULL_SPHERE,
          null,
          targetResolution
        );
        ...
        if (l.levelNumber < 6) {
          expect(preferredCoverage instanceof GebcoElevationCoverage).toBe(
            true
          );
        } else if (l.levelNumber < 10) {
          expect(preferredCoverage instanceof AsterV2ElevationCoverage).toBe(
            true
          );
        } else {
          expect(
            preferredCoverage instanceof UsgsNedElevationCoverage ||
              preferredCoverage instanceof UsgsNedHiElevationCoverage
          ).toBe(true);
        }
      }
    });
  ...
  describe("Min and max elevation calculation tests", function () {
    ...
    it("Disregards elevations for disabled coverages", function () {
      ...
      expect(
        enabledCoverages[enabledCoverages.length - 1].maxElevation
      ).toEqual(em.maxElevation);
    });
